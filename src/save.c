...
void fwrite_char( CHAR_DATA * ch, FILE * fp )
{
   ...
   fprintf( fp, "BIRTHDAY     %d %d %d\n", ch->pcdata->birthday.year,
            ch->pcdata->birthday.month, ch->pcdata->birthday.day );
   fprintf( fp, "AGE          %d\n", ch->pcdata->age );
   ...
   /* v1.54: fighting style persistence */
   if( ch->fstyle != STYLE_STANDARD )
      fprintf( fp, "Fstyle       %d\n", ch->fstyle );
   if( ch->evasive_skill )
      fprintf( fp, "EvasiveSkl   %d\n", ch->evasive_skill );
   if( ch->defensive_skill )
      fprintf( fp, "DefensiveSkl %d\n", ch->defensive_skill );
   if( ch->offensive_skill )
      fprintf( fp, "OffensiveSkl %d\n", ch->offensive_skill );
   if( ch->agressive_skill )
      fprintf( fp, "AgressiveSkl %d\n", ch->agressive_skill );
   ...
   else
   {
      ...
      if( ch->pcdata->clan_name && ch->pcdata->clan_name[0] != '\0' )
         fprintf( fp, "Clan         %s~\n", ch->pcdata->clan_name );
      if( ch->pcdata->clan_rank && ch->pcdata->clan_rank[0] != '\0' )
         fprintf( fp, "ClanRank     %s~\n", ch->pcdata->clan_rank );
      if( ch->pcdata->lastpaid != 0 )
         fprintf( fp, "LastPaid     %ld\n", ch->pcdata->lastpaid );
      fprintf( fp, "Flags        %d\n", ch->pcdata->flags );
...
void fread_char( CHAR_DATA * ch, FILE * fp, bool preload, bool copyover )
{
   ...
   time_t lastplayed = 0;
   int sn, extra __attribute__((unused));

   file_ver = 0;
   killcnt = 0;
   /* Preassign birthday sentinels — get_age will auto-assign on first call
      if these are still -1 after loading (catches pre-birthday player files) */
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'A':
            ...
            KEY( "AgressiveSkl", ch->agressive_skill, fread_number( fp ) );
         ...
         case 'C':
            ...
            KEY( "ClanRank", ch->pcdata->clan_rank, fread_string_nohash( fp ) );
         ...
         case 'D':
            ...
            KEY( "DefensiveSkl", ch->defensive_skill, fread_number( fp ) );
         ...
         case 'H':
            ...
            if( !str_cmp( word, "HpManaMove" ) )
            {
               ...
               if( x4 >= 100 )
               {
                  ch->perm_frc = number_range( 1, 150 );
                  ch->max_mana = x4;
                  ch->mana = x4;
               }
               else if( x4 >= 10 )
         ...
         case 'L':
            ...
            KEY( "LastPaid", ch->pcdata->lastpaid, fread_number( fp ) );
         ...
         case 'O':
            ...
            KEY( "OffensiveSkl", ch->offensive_skill, fread_number( fp ) );
         ...
         case 'S':
            ...
            {
               ch->subclass = fread_number( fp );
               if( ch->subclass < 0 || ch->subclass >= SUBCLASS_MAX )
                  ch->subclass = SUBCLASS_NONE;
               fMatch = TRUE;
               break;
            }
         ...
         case 'E':
            KEY( "EvasiveSkl", ch->evasive_skill, fread_number( fp ) );
            if( !str_cmp( word, "End" ) )
            {
               if( !ch->short_descr )
                  ch->short_descr = STRALLOC( "" );
