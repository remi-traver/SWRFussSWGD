...
void do_connect( CHAR_DATA *ch, const char *argument )
{
 ...
 if ( ( second_ob =  get_obj_carry( ch, arg2 ) )  == NULL )
  ...
  {
     send_to_char( "You cannot connect two of the same object.\n\r", ch );
     return;
  }

  if( first_ob->value[2] == 0 || second_ob->value[2] == 0 ) 
  {
     send_to_char( "Both items must be pieces of another item!\n\r", ch );
     return;
  }
  ...
  /* check to see if the pieces connect */
    /* The or expression confirms a match, the and expression stops 
     * parts from connecting to themselves. */
 ...
 /* good connection  */
  { 
    if( first_ob->value[2] != second_ob->value[2] )
    {
        send_to_char( "These objects do not connect properly, as order seems to matter. Please inform the administration.", ch );
        bug( "Object %d and %d connect together to form different objects.", first_ob->pIndexData->vnum, second_ob->pIndexData->vnum );
        return;
    }
    if ( ( pObjIndex = get_obj_index( first_ob->value[2] ) ) == NULL )
    {
         ...
         bug( "Objects %d and %d connect to form an item that does not exist %d", first_ob->pIndexData->vnum, second_ob->pIndexData->vnum, first_ob->value[2] );
         return;	
    }
    new_ob = create_object( get_obj_index( first_ob->value[2] ), ch->top_level );
    if( !oprog_use_trigger( ch, first_ob, NULL, NULL, NULL ) )
    {
      if( !oprog_use_trigger( ch, second_ob, NULL, NULL, NULL ) )
      {
         act( AT_ACTION, "$n quietly begins to assemble something.\r\n With success, $e builds $p.", ch, new_ob,NULL,  TO_ROOM );
         act( AT_ACTION, "You carefully examine the pieces.\r\n Oh, that's it. You assemble them, creating $p!", ch, new_ob, NULL, TO_CHAR );
      }
    }
    /*
    ...
    separate_obj( first_ob );
    obj_from_char( first_ob );

    separate_obj( second_ob );
    obj_from_char( second_ob );
    switch( new_ob->item_type )
    {
    case ITEM_CONTAINER:
    case ITEM_WORKSPACE:
    case ITEM_KEYRING:
    case ITEM_SPEED_LOADER:
    case ITEM_HOLSTER:
    case ITEM_CORPSE_PC:
    case ITEM_CORPSE_NPC:
    case ITEM_DROID_CORPSE:
       ...
       break;
    default: 
       obj_to_obj( first_ob, new_ob );
       obj_to_obj( second_ob, new_ob );
       break;
    }
  ...
  else
  {
    ...
    act( AT_ACTION, "You try to fit them together every which way, but the damn thing don't want to work right.", ch, NULL, NULL, TO_CHAR);
...
{
   char arg[MAX_INPUT_LENGTH];
   OBJ_DATA *holster, *weapon;
   int weight;
	
   argument = one_argument(argument, arg);
   if(!argument[0])
   {
      send_to_char("&w&RSyntax: draw <weapon> <holster>\n\r", ch);
      return;
   }

   if( (holster = get_obj_carry( ch, argument ) ) == NULL )
   {
      if( ( holster = get_obj_wear( ch, argument ) ) == NULL )
      {
         send_to_char("&w&RYou do not have that item.\n\r", ch);
         return;
      }
   }

   if( holster->item_type != ITEM_HOLSTER )
   {
      send_to_char( "&w&RThats not a holster.\n\r", ch );
      return;
   }
   if( IS_SET(holster->value[1], CONT_CLOSED) )
   {
      send_to_char( "This holster is closed.\n\r", ch );
      return;
   }

   weapon = get_obj_list( ch, arg, holster->first_content );

   if( !weapon )
   {
      send_to_char("That isn't in the holster.\n\r", ch);
      return;
   }

   separate_obj( weapon );

   if ( !CAN_WEAR(weapon, ITEM_TAKE)
   ...
   if ( IS_OBJ_STAT( weapon, ITEM_PROTOTYPE )
   &&  !can_take_proto( ch ) )
   {
       send_to_char( "A godly force prevents you from getting close to it.\n\r", ch );
       return;
   ...
   if ( IS_OBJ_STAT( weapon, ITEM_COVERING ) )
     weight = weapon->weight;
   else
     weight = get_obj_weight( weapon );
   ...
   check_for_trap( ch, weapon, TRAP_GET );

   if ( char_died(ch) || obj_extracted(weapon) )
     return;
  
   act( AT_ACTION, "$n draws $p from $P.", ch, weapon, holster, TO_ROOM );
   act( AT_ACTION, "You draw $p from $P.", ch, weapon, holster, TO_CHAR );

   oprog_get_trigger(ch, weapon);
...
bool    is_rotting      args( ( OBJ_DATA *obj ) );
void restricted_message( CHAR_DATA *ch, int wearloc );

/*
 * how resistant an object is to damage				-Thoric
 */

void holster_message( CHAR_DATA *ch, OBJ_DATA *weapon, OBJ_DATA *holster )
{
   /* Generic holster message - weapon type specific messages need weapons_table */
   switch( weapon->value[3] )
   {
      case WEAPON_LIGHTSABER:
         act( AT_ACTION, "$n deactivates $p and clips it to $P.", ch, weapon, holster, TO_ROOM );
         act( AT_ACTION, "You deactivate $p and clip it to $P.", ch, weapon, holster, TO_CHAR );
         break;
      case WEAPON_VIBRO_SWORD:
      case WEAPON_VIBRO_BLADE:
      case WEAPON_VIBRO_AXE:
         act( AT_ACTION, "$n deactivates $p and sheathes it in $P.", ch, weapon, holster, TO_ROOM );
         act( AT_ACTION, "You deactivate $p and sheath it in $P.", ch, weapon, holster, TO_CHAR );
         break;
      default:
         act( AT_ACTION, "$n holsters $p in $P.", ch, weapon, holster, TO_ROOM );
         act( AT_ACTION, "You holster $p in $P.", ch, weapon, holster, TO_CHAR );
         break;
   }
}


void do_holster( CHAR_DATA *ch, const char *argument )
{
   char arg[MAX_INPUT_LENGTH];
   OBJ_DATA *holster, *weapon, *tmpwield;
   int count;
   int bits;
   ...
   if(!argument[0])
   {
      send_to_char("&w&RSyntax: holster <weapon> <holster>\n\r", ch);
      return;
   }
   ...
   {
      send_to_char("You can't holster that.\n\r", ch);
      return;
   }
   ...
   if( (holster = get_obj_carry( ch, argument ) ) == NULL )
   {
      if( ( holster = get_obj_wear( ch, argument ) ) == NULL )
      {
         send_to_char("&w&RHolster it in what?\n\r", ch);
         return;
      }
   }
   ...
   /* In this codebase weapon type is stored directly in value[3] */
   bits = ( 1 << weapon->value[3] );

   if( !IS_SET(holster->value[5], bits) )
   {
      send_to_char("This holster isn't designed for that weapontype.\n\r", ch);
      return;
   }
   ...
   {
     act( AT_PLAIN, "$p sticks to your fingers.", ch, weapon, NULL, TO_CHAR );
     return;
   }
   count = 0;
   /* I want to know how many items are in that holster already */
   for( tmpwield = holster->first_content; tmpwield; tmpwield = tmpwield->next_content )
      count++; 
   if( count >= holster->value[4] )
   {
      send_to_char( "This holster is already full.\n\r", ch );
      return;
   }

   if ( (get_obj_weight( weapon ) / weapon->count)
      + ((get_obj_weight( holster ) - (holster->weight * holster->count))  / holster->count)
        >  holster->value[0] )
   {
      send_to_char( "It won't fit.\n\r", ch );
      return;
   ...
   if( weapon->wear_loc != WEAR_NONE )
   {
      ...
      && (tmpwield = get_eq_char(ch, WEAR_DUAL_WIELD)) != NULL )
         tmpwield->wear_loc = WEAR_WIELD;
      unequip_char( ch, weapon );
      oprog_remove_trigger( ch, weapon );
      if ( char_died(ch) || obj_extracted(weapon) )
         return;
   }

   separate_obj(weapon);
   separate_obj(holster);
   obj_from_char( weapon );
   weapon = obj_to_obj( weapon, holster );
   check_for_trap ( ch, holster, TRAP_PUT );
   if ( char_died(ch) )
      return;

   count = weapon->count;
   weapon->count = 1;
   holster_message( ch, weapon, holster );
   weapon->count = count;

   if ( IS_SET( sysdata.save_flags, SV_PUT ) )
      save_char_obj(ch);
