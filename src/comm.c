...
void nanny_class_caps_confirm( DESCRIPTOR_DATA *d, const char *argument );
...
void new_descriptor( int new_desc )
{
    ...
    */
   {
      ...
      if( ( gfp = fopen( GREETING_FILE, "r" ) ) != NULL )
      {
         while( ( gc = fgetc( gfp ) ) != EOF && gnum < ( MAX_STRING_LENGTH * 2 - 1 ) )
            gbuf[gnum++] = gc;
         gbuf[gnum] = '\0';
         FCLOSE( gfp );
         send_to_desc_color( gbuf, dnew );
      }
      else
      {
...
void read_from_buffer( DESCRIPTOR_DATA * d )
{
   ...
   for( i = 0, k = 0; d->inbuf[i] != '\n' && d->inbuf[i] != '\r'; i++ )
   {
      ...
      {
         /*
          * This cap used to be a hardcoded 254, far below the actual
          * 1024-byte incomm/inbuf buffers (mud.h: MAX_INPUT_LENGTH,
          * MAX_INBUF_SIZE) - raised so commands that take longer free-form
          * text (idea/bug/typo, etc.) aren't truncated well short of what
          * the buffers can actually hold. The -20 margin leaves room for
          * the null terminator this loop appends below, plus any '!'
          * repeat-command substitution against d->inlast.
          */
...
void nanny( DESCRIPTOR_DATA * d, const char *argument )
{
  ...
  switch( d->connected )
    ...
    case CON_CLASS_CAPS:
      nanny_class_caps_confirm( d, argument );
      break;
...
void nanny_get_new_race( DESCRIPTOR_DATA *d, const char *argument )
{
  ...
  write_to_buffer( d, "\r\nPlease choose a main ability from the following classes, or type help [class] to learn more:\r\n[", 0 );
  buf[0] = '\0';

  for( iClass = 0; iClass < CLASS_MAX; iClass++ )
    {
      if( class_name[iClass] && class_name[iClass][0] != '\0' )
	{
	  if( iClass > 0 )
	    {
	      if( strlen( buf ) + strlen( class_name[iClass] ) > 77 )
		{
		  strlcat( buf, "\r\n", MAX_STRING_LENGTH );
		  write_to_buffer( d, buf, 0 );
		  buf[0] = '\0';
	  ...
	  strlcat( buf, class_name[iClass], MAX_STRING_LENGTH );
	}
    }
...
void nanny_get_new_class( DESCRIPTOR_DATA *d, const char *argument )
{
  ...
  for( iClass = 0; iClass < CLASS_MAX; iClass++ )
    {
      if( toupper( arg[0] ) == toupper( class_name[iClass][0] ) && !str_prefix( arg, class_name[iClass] ) )
	{
  ...
  if( iClass == CLASS_MAX || !class_name[iClass] || class_name[iClass][0] == '\0' )
    {
  ...
  {
    short save_str, save_dex, save_int, save_wis, save_con, save_cha, save_frc;
    int cap, ability;

    save_str = ch->perm_str;
    save_dex = ch->perm_dex;
    save_int = ch->perm_int;
    save_wis = ch->perm_wis;
    save_con = ch->perm_con;
    save_cha = ch->perm_cha;
    save_frc = ch->perm_frc;

    ch->perm_str = 18 + race_table[ch->race].str_plus;
    ...
    ch->perm_cha = 18 + race_table[ch->race].cha_plus;
    ch->perm_frc = 150 + race_table[ch->race].frc_plus;

    snprintf( buf, MAX_STRING_LENGTH,
              "\r\n%s, %s -- theoretical max levels at best possible stats:\r\n",
              race_table[ch->race].race_name, class_name[iClass] );
    write_to_buffer( d, buf, 0 );
    write_to_buffer( d, "----------------------------------------------------------------------------\r\n", 0 );

    for( ability = 0; ability < MAX_ABILITY; ability++ )
    {
       if( !ability_name[ability] || ability_name[ability][0] == '\0' )
          continue;

       if( ability == FORCE_ABILITY )
       {
          snprintf( buf, MAX_STRING_LENGTH, "%-15s   Max: ???%s\r\n",
                    ability_name[ability], ability == iClass ? "   (chosen)" : "" );
          write_to_buffer( d, buf, 0 );
          continue;
       }

       cap = max_level( ch, ability );
       snprintf( buf, MAX_STRING_LENGTH, "%-15s   Max: %-3d%s\r\n",
                 ability_name[ability], cap, ability == iClass ? "   (chosen)" : "" );
       write_to_buffer( d, buf, 0 );
    }

    write_to_buffer( d, "----------------------------------------------------------------------------\r\n", 0 );

    ch->perm_str = save_str;
    ch->perm_dex = save_dex;
    ch->perm_int = save_int;
    ch->perm_wis = save_wis;
    ch->perm_con = save_con;
    ch->perm_cha = save_cha;
    ch->perm_frc = save_frc;
  }

  write_to_buffer( d, "\r\nKeep this class, (Y/N)? ", 0 );
  d->connected = CON_CLASS_CAPS;
}
...
void nanny_read_motd( DESCRIPTOR_DATA *d, const char *argument )
{
  ...
  if( ch->top_level == 0 )
    {
      ...
      ch->perm_frc = number_range( -150, 150 );
	...
	ch->perm_frc = 0;
      else if( ch->race == RACE_DUINUOGWUIN || ch->main_ability == FORCE_ABILITY )
	ch->perm_frc = URANGE( 1, ch->perm_frc, 150 );
      else if( ch->main_ability == CLASS_AMBASSADOR )
	/* v1.30: SWGD hardcodes Ambassador to a flat perm_frc 10 / max_mana 600,
	 * bypassing the force roll entirely. Our chargen instead rolls perm_frc
	 * for everyone and derives max_mana from it (100 + frc*3400/150 below),
	 * so rather than breaking that derivation with a hardcoded mana value,
	 * Ambassador gets the same guaranteed-minimum-force treatment as Force-
	 * sensitive races/classes, floored at 22 - the frc value that produces
	 * ~598 mana under our own formula, matching SWGD's intent as closely as
	 * the two different systems allow. */
	ch->perm_frc = URANGE( 22, ch->perm_frc, 150 );
      else
	ch->perm_frc = URANGE( 0, ch->perm_frc, 150 );
  ...
  if (ch->perm_frc >= 1)
	...
	for( ability = 0; ability < MAX_ABILITY; ability++ )
      ...
      ch->move = ch->max_move;
      if( ch->perm_frc > 0 )
	ch->max_mana = 100 + ( ch->perm_frc * 3400 ) / 150;
      else
	ch->max_mana = 0;
      ch->mana = ch->max_mana;
