...
int get_exp_worth( CHAR_DATA * ch )
{
   ...
   /* if( IS_AFFECTED( ch, AFF_FIRESHIELD ) )
     wexp += ( int )( wexp * 1.2 ); -- disabled, AFF_FIRESHIELD reclaimed for AFF_DARK_FRENZY, see magic.c */
   /* if( IS_AFFECTED( ch, AFF_ICESHIELD ) )
      wexp += ( int )( wexp * 1.2 ); -- disabled, AFF_ICESHIELD reclaimed for AFF_DARK_FATIGUE, see magic.c */ 
...
short get_age( CHAR_DATA * ch )
{
   ...
   /* If birthday hasn't been assigned yet, use the recovery fallback
      and assign today's mud date. get_age in the original SWL does this
      the first time it's called for a character without a birthday. */
   ...
   months = time_info.month - ch->pcdata->birthday.month;
   if( months < 0 )
   {
      years--;
      months += 12;
...
short get_curr_frc( CHAR_DATA * ch )
{
   ...
   max = 150 + race_table[ch->race].frc_plus;
   max = UMIN( max, 150 );
...
/*
 * v1.53: theoretical ceiling for each attribute, for score/mstat's
 * "(max)" display. Each mirrors the exact cap math its get_curr_*
 * counterpart already computes internally (and discards) - factored out
 * here rather than duplicated, so the displayed max can never drift from
 * the ceiling get_curr_* actually enforces.
 */
short get_max_str( CHAR_DATA * ch )
{
   return UMIN( 20 + race_table[ch->race].str_plus, 25 );
}

short get_max_int( CHAR_DATA * ch )
{
   return UMIN( 20 + race_table[ch->race].int_plus, 25 );
}

short get_max_wis( CHAR_DATA * ch )
{
   return UMIN( 20 + race_table[ch->race].wis_plus, 25 );
}

short get_max_dex( CHAR_DATA * ch )
{
   return UMIN( 20 + race_table[ch->race].dex_plus, 25 );
}

short get_max_con( CHAR_DATA * ch )
{
   return UMIN( 20 + race_table[ch->race].con_plus, 25 );
}

short get_max_cha( CHAR_DATA * ch )
{
   return UMIN( 20 + race_table[ch->race].cha_plus, 25 );
}

short get_max_lck( CHAR_DATA * ch )
{
   return UMIN( 20 + race_table[ch->race].lck_plus, 25 );
}

short get_max_frc( CHAR_DATA * ch )
{
   return UMIN( 150 + race_table[ch->race].frc_plus, 150 );
}
...
const char *affect_bit_name( int vector )
{
   ...
   /* if( vector & AFF_FAERIE_FIRE )
      strlcat( buf, " faerie_fire", MAX_STRING_LENGTH ); -- disabled, see magic.c */
   if( vector & AFF_INFRARED )
      strlcat( buf, " infrared", MAX_STRING_LENGTH );
   /* if( vector & AFF_CURSE )
      strlcat( buf, " curse", MAX_STRING_LENGTH ); -- disabled, see magic.c. Bit reclaimed as AFF_REGEN (v1.18) */
   if( vector & AFF_REGEN )
      strlcat( buf, " regenerating", MAX_STRING_LENGTH );
   if( vector & AFF_TRACING )
   ...
   if( vector & AFF_MASKED_AURA )
      strlcat( buf, " masked_aura", MAX_STRING_LENGTH );
   ...
   if( vector & AFF_DARK_FRENZY )
      strlcat( buf, " dark_frenzy", MAX_STRING_LENGTH );
   if( vector & AFF_SHOCKSHIELD )
      strlcat( buf, " shockshield", MAX_STRING_LENGTH );
   if( vector & AFF_DARK_FATIGUE )
      strlcat( buf, " dark_fatigue", MAX_STRING_LENGTH );
   if( vector & AFF_POSSESS )
      strlcat( buf, " possess", MAX_STRING_LENGTH );
...
/* v1.39: ported from SWGD - our version had dropped the actual
 * "<= percent" comparison entirely, so this returned TRUE almost
 * unconditionally regardless of what percent was passed in, making
 * every saves_* check (and anything else routed through chance())
 * effectively always resist/always succeed depending on the caller.
 * Confirmed against SWGD source rather than guessed at. */
 ...
 */
   ...
   ms = 0 - ( ( int )( abs( ch->mental_state ) / 10 ) );
   es = 0 - ( ( int )( abs( ch->emotional_state ) / 10 ) );

   if( ( number_percent(  ) - get_curr_lck( ch ) + 15 - ms - es ) <= percent )
      return TRUE;
   else
      return FALSE;
}
...
{
   short new_percent = percent;

/* v1.39: ported from SWGD - same missing "<= percent" bug as chance()
 * above, plus "attrib" was being used as a raw number added directly
 * into the (broken) comparison rather than as a selector for which
 * stat should reduce the effective percent. No existing callers in
 * this codebase, so safe to bring in line with SWGD's actual design. */
   if( !ch )
   {
      bug( "%s: null ch!", __func__ );
      return FALSE;
   ...
   switch ( attrib )
   {
      default:
         break;
      case ATTRIBUTE_LCK:
         new_percent -= get_curr_lck( ch ) / 2;
         break;
      case ATTRIBUTE_STR:
         new_percent -= get_curr_str( ch ) / 2;
         break;
      case ATTRIBUTE_INT:
         new_percent -= get_curr_int( ch ) / 2;
         break;
      case ATTRIBUTE_DEX:
         new_percent -= get_curr_dex( ch ) / 2;
         break;
      case ATTRIBUTE_CON:
         new_percent -= get_curr_con( ch ) / 2;
         break;
      case ATTRIBUTE_CHA:
         new_percent -= get_curr_cha( ch ) / 2;
         break;
      case ATTRIBUTE_FRC:
         new_percent -= get_curr_frc( ch ) / 2;
         break;
   }
   return chance( ch, new_percent );
}
...
void do_reset_age( CHAR_DATA * ch, const char *argument )
{
   CHAR_DATA *victim;
   char arg[MAX_INPUT_LENGTH];
