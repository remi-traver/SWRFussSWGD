...
int max_level( CHAR_DATA * ch, int ability )
{
   ...
   if( ability == COMBAT_ABILITY )
   {
      ...
      if( ch->main_ability == HUNTING_ABILITY )
         level = 25;
      if( ch->main_ability == CLASS_COMMANDO )
         /* v1.52: was a flat 100 (full Combat). Remi proposed spreading
          * Commando's total point budget across three abilities instead
          * of two - Combat 50 / Smuggling 50 / Bounty Hunting 50 - to
          * match Scoundrel's total spend (75+75=150) now that Commando
          * also has a Smuggling bonus. Same 150-point total as before
          * (100+50), just spread differently. */
         level = 50;
      if( ch->main_ability == CLASS_SCOUNDREL || ch->main_ability == CLASS_SCOUT )
         level = 75;
   ...
   if( ability == ENGINEERING_ABILITY )
   {
      ...
      if( ch->main_ability == PILOTING_ABILITY )
         level = 20;
      if( ch->main_ability == CLASS_CHEMIST )
         level = 75;
      if( ch->race == RACE_WOOKIEE )
         level += 10;
   ...
   if( ability == HUNTING_ABILITY )
   {
      if( ch->main_ability == ability )
         level = 100;
      if( ch->main_ability == CLASS_SNITCH )
         level = 100;
      if( ch->main_ability == CLASS_SCOUT )
         level = 50;
      if( ch->main_ability == CLASS_COMMANDO )
         /* v1.52: new secondary bonus, part of the same rebalance that
          * dropped Commando's Combat cap from 100 to 50 - see the
          * COMBAT_ABILITY block above for the full reasoning. */
         level = 50;
      if( ch->race == RACE_WOOKIEE )
         level += 10;
      if( ch->race == RACE_RODIAN )
   ...
   if( ability == SMUGGLING_ABILITY )
   {
      ...
      if( ch->main_ability == ENGINEERING_ABILITY )
         level = 25;
      if( ch->main_ability == CLASS_SCOUNDREL )
         level = 75;
      if( ch->main_ability == CLASS_SNITCH )
         level = 50;
      if( ch->main_ability == CLASS_COMMANDO )
         /* SWGD grants Commando a flat +50 to Espionage as its secondary
          * ability. This codebase has no Espionage ability/class (excluded
          * per Mark - unfinished even in SWGD), and the old Espionage-guild
          * skills that did get ported here (disguise, hijack, besiege, etc.)
          * mostly landed under Smuggling in skills.dat, so Smuggling is the
          * closest available substitute (v1.51). v1.52: Combat dropped from
          * 100 to 50 and a matching Bounty Hunting 50 added (see
          * COMBAT_ABILITY/HUNTING_ABILITY blocks) so Commando's total point
          * budget is now spread Combat/Smuggling/Bounty Hunting 50/50/50
          * instead of Combat/Smuggling 100/50, matching Scoundrel's total
          * spend at Remi's request. */
         level = 50;
      if( ch->race == RACE_TWI_LEK )
         level += 40;
      if( ch->race == RACE_MON_CALAMARI )
   ...
   {
      /* v1.30: this block did not exist before - MEDICAL_ABILITY had no
       * dedicated cap here at all, so a Medical-class character's own
       * medical skill cap was falling through to the function's level=0
       * default (floored to 1 by the URANGE below). Added following the
       * exact same own-class/cross-class pattern used by every other
       * ability, plus the new Chemist bonus this session added it for. */
      if( ch->main_ability == ability )
         level = 100;
      if( ch->main_ability == CLASS_CHEMIST )
         level = 75;
...
void advance_level( CHAR_DATA * ch, int ability )
{
   ...
   int add_hp, add_mp, add_mv, add_credits;
   ...
   /*
    * Base gains — modified per ability below.
    * HP:   10 + CON modifier from con_app table
    * Move: 10 + DEX/8 + CON/8
    * Mana: 10 + INT/4 + WIS/4  (Force ability only)
    * Credits: 100 * new skill level (non-combat abilities only)
    */
   add_hp      = 10 + con_app[URANGE( 0, get_curr_con( ch ), 25 )].hitp;
   add_mv      = 10 + ( get_curr_dex( ch ) / 8 ) + ( get_curr_con( ch ) / 8 );
   add_mp      = 10 + ( get_curr_int( ch ) / 4 ) + ( get_curr_wis( ch ) / 4 );
   add_credits = 100 * ch->skill_level[ability];

   /* Floor of 1 before ability modifiers */
   add_hp = UMAX( 1, add_hp );
   add_mv = UMAX( 1, add_mv );
   add_mp = UMAX( 1, add_mp );

   /* Clamp base before ability division */
   ...
   {
      case COMBAT_ABILITY:
         /* Full gains — combat is the primary HP/Move ability */
         break;
      ...
      case ENGINEERING_ABILITY:
         ...
         snprintf( buf, MAX_STRING_LENGTH,
                   "Your engineering skills earn you &R%d&W credits! (Total: &R%d&W)\r\n",
                   add_credits, ch->gold );
         send_to_char( buf, ch );
         break;

      case MEDICAL_ABILITY:
         add_hp /= 4;
         add_mv /= 4;
         add_credits *= 2;
         ch->gold += add_credits;
         set_char_color( AT_YELLOW, ch );
         snprintf( buf, MAX_STRING_LENGTH,
                   "Your medical expertise earns you &R%d&W credits! (Total: &R%d&W)\r\n",
                   add_credits, ch->gold );
         send_to_char( buf, ch );
         break;
      ...
      case SMUGGLING_ABILITY:
         ...
         set_char_color( AT_YELLOW, ch );
         snprintf( buf, MAX_STRING_LENGTH,
                   "Your smuggling contacts pay you &R%d&W credits! (Total: &R%d&W)\r\n",
                   add_credits, ch->gold );
         send_to_char( buf, ch );
         break;
      ...
      case LEADERSHIP_ABILITY:
         ...
         set_char_color( AT_YELLOW, ch );
         snprintf( buf, MAX_STRING_LENGTH,
                   "Your leadership position earns you &R%d&W credits! (Total: &R%d&W)\r\n",
                   add_credits, ch->gold );
         send_to_char( buf, ch );
         break;
      ...
      case DIPLOMACY_ABILITY:
         ...
         set_char_color( AT_YELLOW, ch );
         snprintf( buf, MAX_STRING_LENGTH,
                   "Your diplomatic influence earns you &R%d&W credits! (Total: &R%d&W)\r\n",
                   add_credits, ch->gold );
         send_to_char( buf, ch );
         break;

      case FORCE_ABILITY:
         /* Force levels give Mana/Force points instead of HP/Move */
         add_hp /= 5;
         add_mv /= 5;
         ch->max_mana += add_mp;
         set_char_color( AT_CYAN, ch );
         snprintf( buf, MAX_STRING_LENGTH,
                   "Your connection to the Force deepens: &C+%d&W Force Points. (Total: &C%d&W)\r\n",
                   add_mp, ch->max_mana );
         send_to_char( buf, ch );
         break;
   ...
   /* Final floor after ability division */
   add_hp = UMAX( 1, add_hp );
   add_mv = UMAX( 1, add_mv );
   ...
   snprintf( buf, MAX_STRING_LENGTH,
             "You gain &R%d&W HP (&R%d&W total) and &G%d&W Move (&G%d&W total).\r\n",
   ...
   check_force_awakening( ch );
}

/*
 * Force Awakening: a small, flat, runtime-tunable chance on every ability
 * level-up for a character to gain (or deepen) Force sensitivity, separate
 * from the chargen roll and the subclass-selection bonus. Hunters are not
 * excluded here (only from the chargen roll) so they can still discover
 * the Force over the course of a career.
 */
void check_force_awakening( CHAR_DATA * ch )
{
   ...
   if( ( number_range( 1, 1000000 ) / 10000.0 ) > sysdata.force_roll_chance )
      return;

   was_zero = ( ch->perm_frc == 0 );

   ch->perm_frc = URANGE( 0, ch->perm_frc + number_range( 1, 5 ), 150 );

   if( !was_zero )
      send_to_char( "You feel you've connected more with the Force.\r\n", ch );
   else
      ch->max_mana += 700;
}
...
int hit_gain( CHAR_DATA * ch )
{
   ...
   else
   {
      ...
      if( ch->subclass == SUBCLASS_SURVIVOR )
         gain += 20;   /* SWGD survivor regen bonus */
...
void char_update( void )
{
   ...
   for( ch = last_char; ch; ch = gch_prev )
   {
      ...
      /* Wage payment: once per hour of play-time, deduct from clan funds */
      if( !IS_NPC( ch ) && !IS_IMMORTAL( ch ) && ch->pcdata->clan )
      {
         long played_now = ch->played + ( long )( current_time - ch->logon );
         if( ch->pcdata->lastpaid == 0 )
            ch->pcdata->lastpaid = played_now;
         else if( played_now - ch->pcdata->lastpaid >= 3600 )
         {
            int wages = get_wages_for_member( ch->pcdata->clan, ch );
            if( wages > 0 )
            {
               if( ch->pcdata->clan->funds < wages )
                  send_to_char( "&RYour clan cannot afford to pay your wages this period!&w\r\n", ch );
               else
               {
                  ch->pcdata->clan->funds -= wages;
                  ch->gold += wages;
...
void char_check( void )
{
   ...
   for( ch = first_char; ch; ch = ch_next )
   {
      ...
      else
      {
         ...
         {
            if( !IS_AFFECTED( ch, AFF_FLOATING ) && !IS_AFFECTED( ch, AFF_AQUA_BREATH ) && !ch->mount )
            {
               if( get_trust( ch ) < LEVEL_IMMORTAL )
               {
                  int dam;
