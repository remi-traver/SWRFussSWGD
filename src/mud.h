...
/* v1.39: ATTRIBUTE_* - SWGD's chance_attrib() switches on one of these to
 * decide which stat reduces the effective percent. Added because our
 * chance_attrib() previously used its "attrib" argument as a raw number
 * added directly into a broken comparison, rather than as a selector -
 * see the chance()/chance_attrib() fix in handler.c. */
...
typedef enum
{
   ...
   */
   ...
   SUB_SELF_DESTRUCT_1, SUB_SELF_DESTRUCT_2
} char_substates;
...
/* v1.30: flavor/hybrid classes (SWGD "class_type" layer) - these ride an
 * existing ability's skill tree (see max_level() in update.c) rather than
 * being separate trainable abilities. Espionage excluded per Mark - SWGD's
 * own source notes it was never finished ("levels are always 0"). */
#define CLASS_CHEMIST		9
#define CLASS_SCOUNDREL		10
#define CLASS_AMBASSADOR	11
#define CLASS_SCOUT		12
#define CLASS_SNITCH		13
...
#define SUBCLASS_FORCE_SENSITIVE	15
#define SUBCLASS_JEDI		16
#define SUBCLASS_SITH_HUNTER	17
#define SUBCLASS_SOLDIER	18
#define SUBCLASS_MECHANIC	19
#define SUBCLASS_PRODIGY	20
#define SUBCLASS_FALSE_PROPHET	21
#define SUBCLASS_DOCTOR		22
...
typedef enum
{ FIGHTER_SHIP, MIDSIZE_SHIP, FRIGATE_SHIP, CAPITAL_SHIP, SUPERCAPITAL_SHIP, SHIP_PLATFORM, CLOUD_CAR, OCEAN_SHIP, LAND_SPEEDER, WHEELED, LAND_CRAWLER,
WALKER
} ship_classes;
/* v1.22: SUPERCAPITAL_SHIP inserted after CAPITAL_SHIP per SWGD enum order.
 * Saved ship files with Class >= old SHIP_PLATFORM (4) are migrated +1 on
 * load via the SVers key in fread_ship/fwrite_ship (space.c). */

#define MAX_SHIP_MODULES   99   /* SWGD MAX_MODULES */
typedef enum
{ CONCUSSION_MISSILE, PROTON_TORPEDO, HEAVY_ROCKET, HEAVY_BOMB } missile_types;

/* Ship flags — stored in ship->flags */
...
#define IS_DOCKED(ship) ( (ship)->docked_ship && (ship)->shipstate2 == SHIP_DOCK_3 )
...
struct alias_data
{
   ALIAS_DATA  *next;
   ALIAS_DATA  *prev;
   const char  *name;   /* alias keyword */
   const char  *cmd;    /* command it expands to */
};

/*
 * Cargo economy system - v1.19, ported from SWGD (Arcturus). Named
 * cargo types (e.g. "durasteel ore") with a base value; each planet
 * tracks its own resource stock, consumption need, production rate, and
 * current price per type, letting prices float with supply/demand via
 * update_cargo_costs().
 */
#define MAX_CARGO    20
#define CARGO_NONE   0
#define AVG_PLANET_VALUE  50000L   /* baseline planet value tax-factor is computed against */
...
extern CARGO_DATA *cargo_table[MAX_CARGO];
extern int top_cargo_sn;
...
struct planet_data
{
   ...
   bool flags;
   float pop_support;
   int resource[MAX_CARGO];   /* current stock of each cargo type */
   int consumes[MAX_CARGO];   /* how much the planet wants per type */
   int produces[MAX_CARGO];   /* how much the planet produces per type */
   int rcost[MAX_CARGO];      /* current price per ton, recomputed by update_cargo_costs() */
};

#define PLANET_NOCAPTURE  BV00

/* ---- Clan rank / roster system ---- */
#define MAX_CLANRANK    20
#define RANK_LEADER     BV00   /* highest authority, must be 1 per clan */
#define RANK_FLAGS      12     /* total number of flag bits used */

struct rank_data
{
   CLAN_DATA  *clan;    /* back-pointer to owning clan */
   short       slot;   /* permanent id, never changes */
   short       authority; /* 1=top, higher number=lower rank */
   const char *name;   /* display name */
   int         wage;   /* credits paid per hour of play */
};
...
struct roster_data
{
   ...
   const char  *rank_name;  /* rank name at time of last update */
};
...
struct clan_data
{
   ...
   const char *tmpstr;
   /* rank / roster system */
   RANK_DATA   *ranks[MAX_CLANRANK];
   ROSTER_DATA *first_roster;
   ROSTER_DATA *last_roster;
   short        top_rank;   /* index of highest used slot (-1 = no ranks) */
};
...
struct ship_data
{
   ...
   short statet1;
   short statet2;
   short ionstate;   /* v1.20 - ion cannon recharge/damage state, mirrors statet0 for lasers */
   short missiletype;
   short missilestate;
   short torpedostate;  /* v1.25 SWGD: separate torpedo launcher state */
   short rocketstate;   /* v1.25 SWGD: separate rocket launcher state */
   short missiles;
   short maxmissiles;
   short torpedos;
   short maxtorpedos;
   ...
   bool autopilot;
   int flags;   /* SHIPFLAG_* bitfield */
   /* GD extended fields */
   short       shipstate2;      /* secondary state for docking */
   SHIP_DATA  *docked_ship;     /* ship we are docked to */
   short       cloak;           /* cloaking device strength */
   short       overdrive;       /* overdrive node installed */
   short       interdict;       /* interdictor strength */
   short       autocannon;      /* v1.24 SWGD: 1 = autocannon fitted */
   short       autodamage;      /* v1.24 SWGD: damage per autocannon hit */
   short       autoammo;        /* v1.24 SWGD: current autocannon ammo */
   short       autoammomax;     /* v1.24 SWGD: max autocannon ammo */
   bool        interdictactive; /* interdictor currently active */
   short       ions;            /* ion cannon count */
   /* v1.19 - added for ship modules subset 6: none of these existed
    * anywhere in this struct before, unlike every other module-driven
    * stat. */
   short       armor;           /* module-driven hull armor rating */
   short       maxarmor;        /* cap on armor, mirrors the armor/maxX pattern used elsewhere */
   short       cargo;           /* current cargo hold usage */
   short       maxcargo;        /* module-driven cargo capacity */
   short       cargotype;       /* v1.19 - which CARGO_DATA type currently occupies the hold, CARGO_NONE if empty */
   short       laserdamage;     /* module-driven bonus laser damage */
   short       mlaunchers;      /* missile launcher count */
   short       tlaunchers;      /* torpedo launcher count */
   short       rlaunchers;      /* rocket launcher count */
   short       modules;         /* number of modules currently installed */
   short       maxmodules;      /* module slot capacity */
   int         module_vnum[MAX_SHIP_MODULES + 1];   /* installed module vnums, 1-indexed */
   int         sdestnum;        /* self-destruct code (-1 = none) */
};
...
#define AFF_ONFIRE		  BV08   /* Hunter wrist-flamethrower burn DoT - reclaimed from AFF_FAERIE_FIRE */
#define AFF_INFRARED		  BV09
#define AFF_REGEN		  BV10   /* regeneration - reclaimed from AFF_CURSE (v1.18) */
#define AFF_TRACING		  BV11   /* Comlink trace (trace_comlink) - was AFF_FLAMING */
...
#define AFF_MASKED_AURA		  BV19   /* maskaura - reclaimed from AFF_FLYING (v1.18) */
#define AFF_PASS_DOOR		  BV20
#define AFF_FLOATING		  BV21
#define AFF_TRUESIGHT		  BV22
#define AFF_DETECTTRAPS		  BV23
#define AFF_DARK_FRENZY	          BV25   /* dark_frenzy activation */
#define AFF_SHOCKSHIELD	          BV26
#define AFF_SIT_AWARE             BV27 /* situational awareness — alert to sneak attacks */
#define AFF_DARK_FATIGUE  		  BV28   /* dark_frenzy post-expiry crash */
...
typedef enum
{
   ...
   ITEM_MEDICINE, ITEM_SHOT, ITEM_INOCULATOR, ITEM_ELIXIR,
   ITEM_PIECE, ITEM_HOLSTER, ITEM_KEYRING, ITEM_SPEED_LOADER,
   ITEM_WORKSPACE, ITEM_TRAP_KIT, ITEM_FIGHTERCOMP, ITEM_MIDCOMP,
   ITEM_FRIGATECOMP, ITEM_CAPITALCOMP, ITEM_SCAPITALCOMP, ITEM_FUEL_CANISTER, ITEM_FLAMETHROWER, ITEM_JETPACK
} item_types;

#define MAX_ITEM_TYPE		     ITEM_JETPACK

typedef enum
{
   TRAP_MECHANISM_TRIPWIRE = 0, TRAP_MECHANISM_MOTION, TRAP_MECHANISM_HEAT,
   TRAP_MECHANISM_SOUND, TRAP_MECHANISM_PRESSURE
} trap_mechanisms;
#define MAX_TRAP_MECHANISM TRAP_MECHANISM_PRESSURE
/*
* Extra flags.
* Used in #OBJECTS.
*/
...
/* v1.54: fighting styles, ported from SWGD (do_style/learn_style) */
...
typedef enum
{
   ...
   APPLY_HASSEMBLY, APPLY_ENGINE, APPLY_GENERATOR, APPLY_TRACTORBEAM,
   APPLY_OVERDRIVE, APPLY_LASERBATTERY, APPLY_SIMULATOR, APPLY_CLOAK,
   APPLY_MLAUNCHER, APPLY_TLAUNCHER, APPLY_RLAUNCHER,
   /* v1.19 - the rest of SWGD's real ship-stat apply types, appended
    * here (not interspersed) so no existing index shifts. */
   APPLY_HYPERSPEED, APPLY_REALSPEED, APPLY_LASERS, APPLY_MAXSHIELD,
   APPLY_MAXENERGY, APPLY_MAXMISSILES, APPLY_MAXROCKETS, APPLY_MAXTORPEDOS,
   APPLY_COMM, APPLY_SENSOR, APPLY_ASTRO_ARRAY, APPLY_CHAFF,
   APPLY_MANUEVER, APPLY_LASERDAMAGE, APPLY_IONS, APPLY_ARMOR, APPLY_CARGO,
   MAX_APPLY_TYPE
} apply_types;
...
#define PCFLAG_ULTALK              BV13   /* access to ultalk channel */
#define PCFLAG_PCOUNCIL            BV14   /* player council status */
#define PCFLAG_IMMORTAL_ALT        BV15   /* flagged as immortal's alt account */
#define PCFLAG_SPARRING            BV16   /* v1.54: spar mode - see do_spar */
...
#define CHANNEL_ULTALK             BV28
#define CHANNEL_TRANSMIT           BV29
...
struct char_data
{
   ...
   short fstyle;   /* v1.54: fighting style, see STYLE_* defines */
   short evasive_skill;
   ...
   short backup_type;   /* 1=troop squad (follows/charmed), 2=guard (stationary), 3=patrol (wanders) */
   bool backup_elite;   /* standard vs elite stat tier for the above */
   short was_stunned;
...
struct pc_data
{
   ...
   const char *clan_rank;   /* player's current clan rank name */
   long        lastpaid;    /* last time wages were paid (played seconds) */
   const char *pwd;
   const char *email;
   const char *bamfin;
   const char *bamfout;
   ...
   TIME_INFO_DATA birthday;  /* character's birthday in mud-time */
   short age;                /* age offset for qpbuy old/youth */
   int auth_state;
...
struct system_data
{
   ...
   float force_roll_chance;  /* Percent chance per ability level-up of Force Awakening */
...
extern short gsn_inoculate;
extern short gsn_loadinoculator;
extern short gsn_concentrate;
extern short gsn_mix;
extern short gsn_brew;
extern short gsn_loremedicine;
extern short gsn_makemedpac;
extern short gsn_makeflamethrower;
extern short gsn_flamethrower;
extern short gsn_makejetpack;
extern short gsn_jetpackdodge;
extern short gsn_concussion_rocket;
extern short gsn_dark_frenzy;
extern short gsn_makeinoculator;
extern short gsn_makeshot;

extern short gsn_beg;
extern short gsn_makeblade;
extern short gsn_makevibrosword;
...
extern short gsn_deadeye;
extern short gsn_appraiseitem;
extern short gsn_battlecry;
extern short gsn_spray;
extern bool dual_flip;   /* fight.c: v1.31 - now also used by do_spray */

/* v1.18 batch - social/passive Force skills */
extern short gsn_silvertongue;
extern short gsn_dipimmunity;
extern short gsn_maskaura;
extern short gsn_regeneration;

/* v1.18 batch - crafting recipes */
extern short gsn_makeaxe;
extern short gsn_makewhip;
extern short gsn_makebowcaster;
extern short gsn_makeforcepike;
extern short gsn_makedrinkcon;

/* v1.18 batch - engineering item skill */
extern short gsn_improved_concentration;
...
extern short gsn_chain_lightning; /* v1.55 */
...
extern short gsn_vibro_swords;
extern short gsn_flexible_arms;
extern short gsn_talonous_arms;
extern short gsn_bludgeons;
extern short gsn_vibro_axes;

/* Throwing specializations - ported from SWGD (v1.17) */
extern short gsn_saber_throwing;
extern short gsn_dagger_throwing;
extern short gsn_axe_throwing;
extern short gsn_spear_throwing;
extern short gsn_entangling_throw;
extern short gsn_telekinetic_throwing;
/* extern short gsn_flamethrowers; -- intentionally not ported; flamethrower
 * is handled as its own custom weapon/skill (make_flamethrower), not as a
 * SWGD-style weapon-type proficiency. See v1.17 patch notes. */
...
/* v1.54: fstyle terms ported from SWGD verbatim, including SWGD's own
 * quirk of using evasive_skill (not agressive_skill) for the Agressive
 * style's bonus/penalty in all three macros below - that looks like it
 * may be an SWGD bug (agressive_skill is tracked and grown by
 * learn_style() but never actually read anywhere), but since the ask was
 * to port as closely as possible, it's kept as-is rather than silently
 * "fixed". Flagging for a decision whenever balance work touches this. */
...
#define GET_DAMROLL(ch)		((ch)->damroll                              \
   ...
   + ( (ch)->fstyle == STYLE_OFFENSIVE ? (ch)->offensive_skill / 10 : 0 ) \
   + ( (ch)->fstyle == STYLE_AGRESSIVE ? (ch)->evasive_skill / 5    : 0 ) )
#define GET_DAMAGE_RESISTANCE(ch) ( (ch)->subclass == SUBCLASS_TANK ? 25 : 0 )
...
extern FILE *fpArea;
extern FILE *fpReserve;   /* Reserved file handle — kept open as fallback */
void fix_linebreaks  args( ( char *buffer, const char *txt ) );
void fix_amperstands args( ( char *buffer, const char *txt ) );
...
extern const char *const class_name[CLASS_MAX];
...
DECLARE_DO_FUN( do_appraiseitem );
...
DECLARE_DO_FUN( do_situational_awareness );
...
DECLARE_DO_FUN( do_makeaxe );
DECLARE_DO_FUN( do_makewhip );
DECLARE_DO_FUN( do_makebowcaster );
DECLARE_DO_FUN( do_makeforcepike );
DECLARE_DO_FUN( do_makedrinkcon );
DECLARE_DO_FUN( do_makevibrosword );
...
DECLARE_DO_FUN( do_fix_clan_membership );
...
DECLARE_DO_FUN( do_swap_abilities );
DECLARE_DO_FUN( do_toggleimm );
DECLARE_DO_FUN( do_togglenh );
DECLARE_DO_FUN( do_togglesimulator );
...
DECLARE_DO_FUN( do_trigtypes );
DECLARE_DO_FUN( do_tuck );
DECLARE_DO_FUN( do_tune );
DECLARE_DO_FUN( do_ultalk );
DECLARE_DO_FUN( do_make_home );
DECLARE_DO_FUN( do_sell_home );
DECLARE_DO_FUN( do_set_home );
DECLARE_DO_FUN( do_homes );
DECLARE_DO_FUN( do_initialize_homes );
DECLARE_DO_FUN( do_reset_all_homes );
...
/* DECLARE_SPELL_FUN( spell_curse );  -- disabled, see magic.c */
...
/* DECLARE_SPELL_FUN( spell_faerie_fire );  -- disabled, see magic.c */
...
DECLARE_SPELL_FUN( spell_dark_frenzy );
...
#define VERSION_FILE	SYSTEM_DIR "version.txt"
#define GREETING_FILE	SYSTEM_DIR "greeting.txt"
#define NULL_FILE	"/dev/null"    /* Reserved file handle  */
#define HELP_WEB_DIR	"../public_html/helps/"  /* Web help export dir  */
...
void save_ranks args( ( CLAN_DATA * clan ) );
void save_roster args( ( CLAN_DATA * clan ) );
void load_ranks args( ( CLAN_DATA * clan ) );
void load_roster args( ( CLAN_DATA * clan ) );
void update_roster args( ( CHAR_DATA * ch, int action ) );
int  add_rank args( ( CLAN_DATA * clan ) );
int  get_rank_by_name args( ( CLAN_DATA * clan, const char *name ) );
ROSTER_DATA *get_roster_by_name args( ( CLAN_DATA * clan, const char *name ) );
int  get_wages_for_member args( ( CLAN_DATA * clan, CHAR_DATA * ch ) );
...
float speedbonus     args( ( SHIP_DATA * ship ) );
bool  check_sabotage args( ( SHIP_DATA * ship, int check ) );
void  rotate_ship    args( ( SHIP_DATA * ship, double angle_x, double angle_y, double angle_z ) );
void  adjust_bay     args( ( SHIP_DATA * ship, bool fOpen ) );
long  prepcost       args( ( SHIP_DATA * ship ) );
void  prepship       args( ( SHIP_DATA * ship ) );
void  initialize_code args( ( char *buf ) );
...
bool check_parry( CHAR_DATA * ch, CHAR_DATA * victim, int dt, int dam );
bool check_dodge( CHAR_DATA * ch, CHAR_DATA * victim );
bool check_duck( CHAR_DATA * ch, CHAR_DATA * victim );
bool check_deflect( CHAR_DATA * ch, CHAR_DATA * victim, int dt, int dam );
int throwing_mastery_skill( int weapon_type );
bool is_melee_weapon( int weapon_type );
...
/* v1.53: theoretical ceiling for each attribute (race-adjusted, same cap
 * math get_curr_* already clamps against), for score/mstat "(max)" display. */
short get_max_str args( ( CHAR_DATA * ch ) );
short get_max_int args( ( CHAR_DATA * ch ) );
short get_max_wis args( ( CHAR_DATA * ch ) );
short get_max_dex args( ( CHAR_DATA * ch ) );
short get_max_con args( ( CHAR_DATA * ch ) );
short get_max_cha args( ( CHAR_DATA * ch ) );
short get_max_lck args( ( CHAR_DATA * ch ) );
short get_max_frc args( ( CHAR_DATA * ch ) );
...
ALIAS_DATA *find_alias args( ( CHAR_DATA * ch, const char *name ) );
...
void holster_message args( ( CHAR_DATA * ch, OBJ_DATA * weapon, OBJ_DATA * holster ) );
...
void check_force_awakening( CHAR_DATA * ch );
