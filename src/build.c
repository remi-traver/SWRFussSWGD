...
const char *const planet_flags[] = {
   ...
   "chad", "korriban", "tython", "p18", "p19", "p20", "p21", "p22", "p23", "p24",
...
const char *const weapon_table[13] = {
   ...
   "blaster", "vibro-sword", "bludgeon", "bowcaster", "w10",
   "force pike", "w12"
};
...
const char *const o_types[] = {
   ...
   "medicine", "shot", "inoculator", "elixir",
   "piece", "holster", "keyring", "speed_loader", "workspace", "trap_kit",
   "fightercomp", "midcomp", "frigatecomp", "capitalcomp", "scapitalcomp", "fuel_canister", "flamethrower", "jetpack"
};
...
const char *const a_types[] = {
   ...
   "hassembly", "engine", "generator", "tractorbeam", "overdrive",
   "laserbattery", "simulator", "cloak", "mlauncher", "tlauncher", "rlauncher",
   "hyperspeed", "realspeed", "lasers", "maxshield",
   "maxenergy", "maxmissiles", "maxrockets", "maxtorpedos",
   "comm", "sensor", "astro_array", "chaff",
   "manuever", "laserdamage", "ions", "armor", "cargo"
};
...
const char *const pc_flags[] = {
   ...
   "nosummon", "pageron", "notitled", "room", "r12", "r13", "r14", "r15", "sparring", "r17", "r18", "r19",
...
int get_npc_position( const char *position )
{
   ...
   /*
    * npc_position[] has more string entries than the real POS_ enum has
    * values (POS_DRAG caps this loop at 11), so "standing"/"mounted"/
    * "shove"/"drag" sit past the reachable range and can never match here.
    * Every existing mob in every area file uses "aggressive" (index 8) for
    * Position/DefPos, which happens to equal the real POS_STANDING (8) -
    * that's the de facto working convention this whole project relies on.
    * Rather than touch npc_position[] or the loop bound (which would
    * silently reassign every existing mob's position), "standing" is
    * special-cased here as an explicit synonym for that same value, so
    * new content can use the semantically correct word without altering
    * anything already in production. mounted/shove/drag remain unreachable
    * pending a deliberate, separate audit - see project notes.
    */
   if( !str_cmp( position, "standing" ) )
      return POS_STANDING;
      ...
      case ITEM_WEAPON:
	   ...
	   size_t x;
            ...
            if( value < 0 )
            {
               ...
               send_to_char( "\r\nChoices:\r\n", ch );
               send_to_char( "   none, lightsaber, vibro-blade, vibro-sword, blaster, force pike, bowcaster, bludgeon\r\n", ch );
               return;
            }
            tmp = 3;
            break;
