...
void do_mstat( CHAR_DATA * ch, const char *argument )
{
   ...
   ch_printf( ch, "Str: %2d (%2d)  Int: %2d (%2d)  Wis: %2d (%2d)  Dex: %2d (%2d)  Con: %2d (%2d)  Cha: %2d (%2d)  Lck: %2d (%2d)  Frc: %2d (%2d)\r\n",
              ...
              get_curr_cha( victim ), get_max_cha( victim ),
              get_curr_lck( victim ), get_max_lck( victim ),
              get_curr_frc( victim ), get_max_frc( victim ) );
...
void do_balzhur( CHAR_DATA* ch, const char* argument )
{
   ...
   for( pArea = first_build; pArea; pArea = pArea->next )
   {
      if( !str_cmp( pArea->filename, areafile ) )
      {
         ...
         bc = snprintf( buildbackup, 260, "%s.bak", buildfile );
         if( bc < 0 )
            bug( "%s: Output buffer error!", __func__ );
...
/* Promote an immortal one level up. Max reachable via command is 104 (LEVEL_ETERNAL).
 * Level 105 (LEVEL_SUPREME) must be set manually in the player file — by design.
 * Only a level 105 character can use this command.
 */
void do_immpromote( CHAR_DATA * ch, const char *argument )
{
   char arg[MAX_INPUT_LENGTH];
   CHAR_DATA *victim;
   ...
   if( arg[0] == '\0' )
   {
      send_to_char( "Syntax: immpromote <player>\r\n", ch );
      return;
   }

   if( get_trust( ch ) < LEVEL_SUPREME )
   {
      send_to_char( "Only a Supreme (level 105) can use this command.\r\n", ch );
      return;
   }
   ...
   {
      send_to_char( "Use 'immortalize' to first raise a mortal to immortal status.\r\n", ch );
      return;
   }

   if( victim->top_level >= LEVEL_ETERNAL )
   {
      send_to_char( "That player is already at the highest level reachable by command (104).\r\n"
                    "Level 105 must be granted manually in the player file.\r\n", ch );
      return;
   }

   victim->top_level++;
   victim->trust = 0;   /* clear any trust override so top_level is authoritative */

   ch_printf( ch, "You promote %s to level %d.\r\n", victim->name, victim->top_level );
   ch_printf( victim, "%s has promoted you to level %d.\r\n", ch->name, victim->top_level );

   switch( victim->top_level )
   {
      case LEVEL_IMMORTAL: ch_printf( victim, "You have become a Neophyte Immortal!\r\n" ); break;
      case LEVEL_CREATOR:  ch_printf( victim, "You have become a Creator!\r\n" );           break;
      case LEVEL_GOD:      ch_printf( victim, "You have become a God!\r\n" );               break;
      case LEVEL_ETERNAL:  ch_printf( victim, "You have become an Eternal!\r\n" );          break;
      default: break;
   }
...
/* Demote an immortal one level down.
 * Cannot demote below LEVEL_IMMORTAL (101) — use 'outcast' or manual edit for that.
 * Only a level 105 character can use this command.
 */
void do_immdemote( CHAR_DATA * ch, const char *argument )
{
   char arg[MAX_INPUT_LENGTH];
   CHAR_DATA *victim;
   ...
   if( arg[0] == '\0' )
   {
      send_to_char( "Syntax: immdemote <player>\r\n", ch );
      return;
   }
   ...
   {
      send_to_char( "You cannot demote another Supreme — edit the player file directly.\r\n", ch );
      return;
   }

   if( victim->top_level <= LEVEL_IMMORTAL )
   {
      send_to_char( "That player is already at the lowest immortal level (101).\r\n"
                    "To remove immortal status entirely, edit the player file directly.\r\n", ch );
      return;
   }

   victim->top_level--;
   victim->trust = 0;   /* clear any trust override */

   ch_printf( ch, "You demote %s to level %d.\r\n", victim->name, victim->top_level );
   ch_printf( victim, "%s has demoted you to level %d.\r\n", ch->name, victim->top_level );
...
void do_award( CHAR_DATA * ch, const char *argument )
{
   ...
   if( !str_cmp( arg1, "all" ) )
   {
      ...
      ch_printf( ch, "&wYou have awarded everyone &Y%d&w Quest Points!\r\n", value );
      ...
      for( vch = first_char; vch; vch = vch_next )
      {
         ...
         if( !IS_NPC( vch ) && !IS_IMMORTAL( vch ) )
         {
            if( value > 0 )
            {
               ...
               ch_printf( vch, "&wYou have been awarded &Y%d&w Quest Points by %s!\r\n", value, ch->name );
            }
            else
            {
               vch->pcdata->quest_curr += value;
               ch_printf( vch, "&wYou have been penalized &Y%d&w Quest Points by %s!\r\n", value, ch->name );
            }
            do_save( vch, "" );
         }
      }
   ...
   if( IS_NPC( victim ) )
   {
      send_to_char( "&YNot on NPCs!\r\n", ch );
      return;
   }

   if( ch == victim )
   {
      send_to_char( "&YYou may not award yourself!\r\n", ch );
      return;
   }
   ...
   ch_printf( ch, "&wYou have awarded %s &Y%d&w Quest Points!\r\n", victim->name, value );

   if( value > 0 )
   {
      victim->pcdata->quest_curr += value;
      victim->pcdata->quest_accum += value;
      ch_printf( victim, "&wYou have been awarded &Y%d&w Quest Points by %s!\r\n", value, ch->name );
   }
   else
   {
      victim->pcdata->quest_curr += value;
      ch_printf( victim, "&wYou have been penalized &Y%d&w Quest Points by %s!\r\n", value, ch->name );
   }
   do_save( victim, "" );
}

void do_trainall( CHAR_DATA * ch, const char *argument )
{
   CHAR_DATA *victim;
   char arg[MAX_INPUT_LENGTH];
   int sn, count;
   ...
   if( arg[0] == '\0' )
   {
      send_to_char( "Syntax: trainall <player>\r\n", ch );
      return;
   }
   ...
   {
      send_to_char( "You cannot train someone of equal or higher trust than yourself.\r\n", ch );
      return;
   }
   ...
   for( sn = 0; sn < top_sn; sn++ )
   {
      ...
      if( skill_table[sn]->guild < 0 || skill_table[sn]->guild >= MAX_ABILITY )
         continue;
      /* Only train skills the player qualifies for based on their ability levels */
   ...
   ch_printf( ch, "You have trained %s in %d skills to 100%%.\r\n", victim->name, count );
   ch_printf( victim, "%s has trained you in all of your available skills to 100%%.\r\n", ch->name );
}
...
void do_viewskills( CHAR_DATA * ch, const char *argument )
{
   ...
   if( IS_NPC( victim ) )
   {
      send_to_char( "&RMobs don't have skills.&w\r\n", ch );
      return;
   }
...
void do_antirestore( CHAR_DATA * ch, const char *argument )
{
   char arg[MAX_INPUT_LENGTH];
   char buf[MAX_STRING_LENGTH];
   CHAR_DATA *victim;
      ...
      for( vch = first_char; vch; vch = vch_next )
      {
         ...
         if( !IS_NPC( vch ) && !IS_IMMORTAL( vch ) )
         {
            ...
            snprintf( buf, MAX_STRING_LENGTH,
                      "&B%s&z&w has &BA&zn&BT&zi&BR&ze&BS&zt&BO&zr&BE&zd&w %s&z!!!",
                      ch->name, vch->name );
...
void do_smite( CHAR_DATA * ch, const char *argument )
{
   ...
   snprintf( buf, MAX_STRING_LENGTH,
             "%s lashes out at %s with a smiting from the heavens!", ch->name, victim->name );
   echo_to_all( AT_IMMORT, buf, ECHOTAR_ALL );

   victim->max_hit  = ( int )( victim->max_hit  * 0.90 );
   if( victim->max_mana > 0 )
      victim->max_mana = ( int )( victim->max_mana * 0.90 );
   victim->max_move = ( int )( victim->max_move * 0.90 );
   victim->hit  = 0;
   victim->mana = 0;
   victim->move = 0;
}

void do_toggleut( CHAR_DATA * ch, const char *argument )
{
   char arg[MAX_INPUT_LENGTH];
   CHAR_DATA *victim;
   ...
   if( !*arg )
   {
      send_to_char( "Toggle ultalk for whom?\r\n", ch );
      return;
   }
   ...
   if( !IS_SET( victim->pcdata->flags, PCFLAG_ULTALK ) )
   {
      SET_BIT( victim->pcdata->flags, PCFLAG_ULTALK );
      ch_printf( victim, "%s has awarded you the UltimateTalk channel. Syntax: 'ultalk <message>'.\r\n", ch->name );
   }
   else
   {
      REMOVE_BIT( victim->pcdata->flags, PCFLAG_ULTALK );
      ch_printf( victim, "%s has revoked your UltimateTalk privileges.\r\n", ch->name );
   }
   save_char_obj( victim );
}

void do_togglenh( CHAR_DATA * ch, const char *argument )
{
   char arg[MAX_INPUT_LENGTH];
   CHAR_DATA *victim;
   ...
   if( !*arg )
   {
      send_to_char( "Toggle player council for whom?\r\n", ch );
      return;
   }
   if( !( victim = get_char_world( ch, arg ) ) || IS_NPC( victim ) )
   {
   ...
   if( !IS_SET( victim->pcdata->flags, PCFLAG_PCOUNCIL ) )
   {
      SET_BIT( victim->pcdata->flags, PCFLAG_PCOUNCIL );
      ch_printf( victim, "%s has awarded you Player Council status.\r\n", ch->name );
   }
   else
   {
      REMOVE_BIT( victim->pcdata->flags, PCFLAG_PCOUNCIL );
      ch_printf( victim, "%s has revoked your Player Council status.\r\n", ch->name );
   }
   save_char_obj( victim );
}

void do_toggleimm( CHAR_DATA * ch, const char *argument )
{
   char arg[MAX_INPUT_LENGTH];
   CHAR_DATA *victim;
   ...
   if( !*arg )
   {
      send_to_char( "Mark whom as an immortal alt?\r\n", ch );
      return;
   }
   if( !( victim = get_char_world( ch, arg ) ) || IS_NPC( victim ) )
   {
   ...
   if( !IS_SET( victim->pcdata->flags, PCFLAG_IMMORTAL_ALT ) )
   {
      SET_BIT( victim->pcdata->flags, PCFLAG_IMMORTAL_ALT );
      ch_printf( victim, "%s has marked you as an immortal's alt.\r\n", ch->name );
   }
   else
   {
      REMOVE_BIT( victim->pcdata->flags, PCFLAG_IMMORTAL_ALT );
      ch_printf( victim, "%s has removed your immortal alt marking.\r\n", ch->name );
   }
   save_char_obj( victim );
}
...
void do_showweather( CHAR_DATA * ch, const char *argument )
{
   ...
   ch_printf( ch, "Current global weather conditions:\r\n" );
   set_char_color( AT_WHITE, ch );
   ch_printf( ch, "  Pressure : %d mmhg (change %d)\r\n", weather_info.mmhg, weather_info.change );
   ch_printf( ch, "  Sky      : %d\r\n", weather_info.sky );
   ch_printf( ch, "  Sunlight : %d\r\n", weather_info.sunlight );
}

void save_sysdata args( ( SYSTEM_DATA sys ) );

void do_setforceroll( CHAR_DATA * ch, const char *argument )
{
   char arg[MAX_INPUT_LENGTH];
   const char *p;
   bool seen_dot = FALSE;
   ...
   if( arg[0] == '\0' )
   {
      ch_printf( ch, "Current Force awakening chance: %g%%.\r\n", sysdata.force_roll_chance );
      return;
   }
   ...
   for( p = arg; *p != '\0'; p++ )
   {
      ...
      {
         send_to_char( "Setforceroll: value must be a number (fractions like .5 or .0001 are fine).\r\n", ch );
         return;
      }
   }
   ...
   sysdata.force_roll_chance = value;
   save_sysdata( sysdata );

   ch_printf( ch, "Force awakening chance set to %g%%.\r\n", value );
   log_string_plus( "Force roll chance changed.", LOG_NORMAL, get_trust( ch ) );
}

void do_fix_skills( CHAR_DATA * ch, const char *argument )
{
   int sn;
   ...
   for( sn = 0; sn < top_sn; sn++ )
   {
      ...
      /* Fix skills with null spell_fun (should have skill_fun instead) */
      if( skill_table[sn]->spell_fun == spell_null )
      {
         skill_table[sn]->spell_fun = NULL;
         if( !skill_table[sn]->skill_fun )
            skill_table[sn]->skill_fun = do_disabled;
         continue;
      }
   }
   send_to_char( "Done.\r\n", ch );
...
void do_reset_all_ships( CHAR_DATA * ch, const char *argument )
{
   SHIP_DATA *ship;
   ...
   for( ship = first_ship; ship; ship = ship->next )
   {
      switch( ship->type )
      {
         ...
         case SHIP_REPUBLIC:
            ...
            {
               STRFREE( ship->owner );
               ship->owner = STRALLOC( "New Republic" );
            }
            STRFREE( ship->pilot );   STRFREE( ship->copilot );
            ship->pilot = STRALLOC( "" );   ship->copilot = STRALLOC( "" );
            break;
         case SHIP_IMPERIAL:
            if( str_cmp( ship->owner, "The Empire" ) )
            {
               STRFREE( ship->owner );
               ship->owner = STRALLOC( "The Empire" );
            }
            STRFREE( ship->pilot );   STRFREE( ship->copilot );
            ship->pilot = STRALLOC( "" );   ship->copilot = STRALLOC( "" );
            break;
...
void do_swap_abilities( CHAR_DATA * ch, const char *argument )
{
   char arg1[MAX_INPUT_LENGTH];
   int sn, first_ability, second_ability;

   argument = one_argument( argument, arg1 );

   if( !is_number( arg1 ) || !is_number( argument ) )
   {
      send_to_char( "Both ability types must be numeric.\r\n", ch );
      return;
   }
   first_ability  = atoi( arg1 );
   second_ability = atoi( argument );

   if( ( first_ability  < 0 || first_ability  >= MAX_ABILITY )
   ||  ( second_ability < 0 || second_ability >= MAX_ABILITY ) )
   {
      ch_printf( ch, "Valid range is 0 to %d.\r\n", MAX_ABILITY - 1 );
      return;
   }

   for( sn = 0; sn < top_sn; sn++ )
   {
      if( skill_table[sn]->guild == first_ability )
         skill_table[sn]->guild = second_ability;
      else if( skill_table[sn]->guild == second_ability )
         skill_table[sn]->guild = first_ability;
   }
   send_to_char( "Done.\r\n", ch );
}
   ...
   if( !rename( pfile, backup ) )
   {
      ...
      for( pArea = first_build; pArea; pArea = pArea->next )
      {
         if( !str_cmp( pArea->filename, areafile ) )
         {
            ...
            snprintf( buildbackup, 260, "%s.bak", buildfile );
            set_char_color( AT_RED, ch ); /* Log message changes colors */
...
void add_command( CMDTYPE * command )
{
   ...
   /*
    * NOTE: when a bucket is non-empty, new commands are APPENDED to the
    * tail below (search "add to the END of the list") - so bucket
    * traversal order matches commands.dat's file order. The FIRST entry
    * of a given starting letter in that file is checked FIRST by
    * interpret()'s prefix match and wins single-letter/abbreviated input.
    * commands.dat relies on this: north/south/east/west/up/down are
    * deliberately kept as the FIRST entry of their letter so bare
    * "n"/"s"/"e"/"w"/"u"/"d" always resolves to movement. Adding a new
    * same-letter command before one of those in commands.dat will
    * silently break that - move the movement command back up if so.
    *
    * The same rule applies to the diagonal movement abbreviations
    * "ne"/"nw"/"se"/"sw" (northeast/northwest/southeast/southwest) and to
    * any other short/abbreviated command name (e.g. "sc" for score):
    * they must precede ANY other command in commands.dat that starts
    * with the same letters, not just be first for the leading letter.
    * Real bug hit in the field: "se" resolved to "setrank" and "sc"
    * resolved to "scrapattack" because those longer commands happened
    * to sit earlier in commands.dat. When adding or reordering commands,
    * check every existing short/abbreviated command name against the
    * new one's prefix and re-sort if there's a collision.
    */
...
void do_exitflags(CHAR_DATA *ch, const char *argument )
{
  int loc;
  ch_printf(ch, "Exit flags are as follows.\n\r");
  for( loc = 0; loc < MAX_EXFLAG; loc++ )
  {
     ch_printf(ch, "%-15s ", ex_flags[loc]);
     if( (loc+1) %4 == 0 )
       ch_printf(ch, "\n\r");
  }
  return;
...
void do_keytypes( CHAR_DATA *ch, const char *argument )
{
   send_to_char( "Lock flags: EX_LOCKED EX_ISDOOR EX_CLOSED EX_SECRET\r\n", ch );
   send_to_char( "            EX_SWIM EX_FLY EX_CLIMB EX_WINDOW\r\n", ch );
   send_to_char( "Set via redit exit with the 'door', 'secret', 'locked' flags.\r\n", ch );
}

void do_reset_all_homes( CHAR_DATA *ch, char *argument )
{
    int iHash = 0;
    ...
    for ( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
    {
              ...
              pRoomIndex  = pRoomIndex->next )
        {
           ...
           {
              sprintf( buf, "Home in room %d reset.", pRoomIndex->vnum );
              log_string( buf ); 
              REMOVE_BIT(pRoomIndex->room_flags, ROOM_PLR_HOME);
              SET_BIT(pRoomIndex->room_flags, ROOM_EMPTY_HOME);
              STRFREE( pRoomIndex->name );
              pRoomIndex->name = STRALLOC( "Empty Home" );
           }
        }
    }
    do_foldarea( ch, "all" );
    /* HOME_DATA loop omitted - homes system not yet implemented */
    send_to_char( "Done.\n\r", ch );
    return;
}

void do_triggers( CHAR_DATA * ch, const char *argument )
{

  int loc;
  ch_printf(ch, "Trigger flags are as follows\n\r");
  for( loc = 0; loc < 32; loc++ )
  {
     ch_printf(ch, "%-15s ", trig_flags[loc]);
     if( (loc+1) %4 == 0 )
       ch_printf(ch, "\n\r");
  }
  return;
...
void do_trigtypes( CHAR_DATA * ch, const char *argument )
{

  int loc;
  ch_printf(ch, "Push/pull triggers marked with a * have 0 functionality.\n\r");
  for( loc = 0; loc < 32; loc++ )
  {
     ch_printf(ch, "%-15s ", trig_flags[loc]);
     if( (loc+1) %4 == 0 )
