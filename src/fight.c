...
void violence_update( void )
{
   ...
   for( ch = last_char; ch; lst_ch = ch, ch = gch_prev )
   {
      ...
      for( paf = ch->first_affect; paf; paf = paf_next )
      {
         ...
         if( paf->bitvector == AFF_ONFIRE && !char_died( ch ) )
         {
            int burn_dam = UMAX( 1, paf->modifier );

            act( AT_FIRE, "$n writhes in flame!", ch, NULL, NULL, TO_ROOM );
            ch_printf( ch, "&RYou're currently on fire! &W[&R%d&W]&w\r\n", burn_dam );
            damage( ch, ch, burn_dam, gsn_flamethrower );
            if( char_died( ch ) )
               break;
         }
         else if( paf->bitvector == AFF_DARK_FATIGUE && !char_died( ch ) )
         {
            int crash_dam = UMAX( 1, paf->modifier );

            act( AT_RED, "$n shudders, spent from the dark side's fury.", ch, NULL, NULL, TO_ROOM );
            ch_printf( ch, "&RYou shudder, spent from the dark side's fury! &W[&R%d&W]&w\r\n", crash_dam );
            damage( ch, ch, crash_dam, gsn_dark_frenzy );
            if( char_died( ch ) )
               break;
         }
         ...
         else
         {
            ...
            if( paf->bitvector == AFF_DARK_FRENZY )
            {
               AFFECT_DATA crash;
               double fraction = UMIN( 1.0, ( ch->skill_level[COMBAT_ABILITY] + ch->skill_level[FORCE_ABILITY] ) / 300.0 );

               crash.type = paf->type;
               crash.duration = number_range( 2, 4 );
               crash.location = APPLY_NONE;
               crash.modifier = ( int )( ch->max_hit * ( 0.02 + fraction * 0.04 ) );
               crash.bitvector = AFF_DARK_FATIGUE;
               affect_to_char( ch, &crash );
               act( AT_RED, "The dark side's fury drains from $n, leaving $m spent and shaking.", ch, NULL, NULL, TO_ROOM );
               send_to_char( "The dark side's fury drains from you, leaving you spent and shaking.\r\n", ch );
            }
...
ch_ret multi_hit( CHAR_DATA * ch, CHAR_DATA * victim, int dt )
{
   ...
   if( IS_AFFECTED( ch, AFF_DARK_FRENZY ) && who_fighting( ch ) == victim )
   {
      AFFECT_DATA *paf;
      int bonus_attacks = 0;
   ...
   /* Martial Artists dual-wield with their fists */
   if( ch->subclass == SUBCLASS_MARTIST && get_eq_char( ch, WEAR_WIELD ) == NULL )
   {
      ...
      if( retcode != rNONE || who_fighting( ch ) != victim )
         return retcode;
      /* One more attack for their speed bonus */
      retcode = one_hit( ch, victim, dt );
      if( retcode != rNONE || who_fighting( ch ) != victim )
         return retcode;
   }
   ...
   schance = IS_NPC( ch ) ? ch->top_level : ( int )( ( ch->pcdata->learned[gsn_fourth_attack] + ( dual_bonus * 1.5 ) ) / 2 );
   if( number_percent(  ) < schance )
   {
   ...
   schance = IS_NPC( ch ) ? ch->top_level : ( int )( ( ch->pcdata->learned[gsn_fifth_attack] + ( dual_bonus * 1.5 ) ) / 2 );
   if( number_percent(  ) < schance )
   {
 ...
 *
 * SWGD keyed these off a separate weapons_table[] struct (per-weapon
 * flags, attack messages, etc.) that doesn't exist in this codebase.
 * Adapted here to key off the existing value[3] weapon-type constants
 * used by weapon_prof_bonus_check() above, so no new infrastructure is
 * required. Weapon types with no natural "throwing" analog (blaster,
 * bowcaster, bludgeon, claw, vibro-sword) intentionally return -1: no
 * mastery skill applies to them.
 */
int throwing_mastery_skill( int weapon_type )
{
   switch ( weapon_type )
   {
      case WEAPON_LIGHTSABER:
         return gsn_saber_throwing;
      case WEAPON_VIBRO_BLADE:
         return gsn_dagger_throwing;
      case WEAPON_VIBRO_AXE:
         return gsn_axe_throwing;
      case WEAPON_FORCE_PIKE:
         return gsn_spear_throwing;
      case WEAPON_WHIP:
         return gsn_entangling_throw;
      default:
         return -1;
   }
}
...
/*
 * Is this weapon type a melee weapon (as opposed to a ranged weapon like
 * blasters/bowcasters)? Used to gate telekinetic_throwing - you can pull
 * a thrown blade back to your hand, but not a blaster bolt.
 */
...
ch_ret one_hit( CHAR_DATA * ch, CHAR_DATA * victim, int dt )
{
   ...
   /* Martial Artists hit significantly harder unarmed */
   if( !IS_NPC( ch ) && ch->subclass == SUBCLASS_MARTIST && !wield )
   ...
   /* if( IS_AFFECTED( victim, AFF_FIRESHIELD ) && !IS_AFFECTED( ch, AFF_FIRESHIELD ) )
      retcode = spell_fireball( gsn_fireball, off_shld_lvl( victim, ch ), victim, ch ); -- disabled, AFF_FIRESHIELD reclaimed for AFF_DARK_FRENZY, see magic.c */
   if( retcode != rNONE || char_died( ch ) || char_died( victim ) )
      return retcode;
...
ch_ret damage( CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt )
{
   ...
   /* v1.54: captured for do_spar's sparring safety net below - the hit
    * total before this hit's damage is applied. */
   init_hit = victim->hit;
   ...
   if( victim != ch )
   {
      ...
      if( dt >= TYPE_HIT )
      {
         ...
         if( check_parry( ch, victim, dt, dam ) )
            return rNONE;
   ...
   /*
    * Apply damage resistance (Tank subclass)
    */
   if( !IS_NPC( victim ) && dam > 0 )
      dam = UMAX( 0, dam - GET_DAMAGE_RESISTANCE( victim ) );
   ...
   /*
    * Entangling throw - ported from SWGD (v1.17). Whip/chain weapons
    * thrown via entangling_throw wrap around the victim: -6 DEX for 8
    * rounds, plus an immediate movement drain scaled by hit damage
    * (same encumbrance() helper used elsewhere in this function).
    */
   if( dt == gsn_entangling_throw && dam )
   {
      AFFECT_DATA af;

      af.type = gsn_entangling_throw;
      af.duration = 8;
      af.location = APPLY_DEX;
      af.modifier = -6;
      af.bitvector = 0;
      affect_join( victim, &af );
      victim->move = UMAX( 0, ( victim->move - encumbrance( victim, dam ) ) );
   }

   /* v1.54: ported from SWGD. If both combatants have spar mode on, the
    * loser is knocked to 1hp and combat stops instead of them dying.
    * SWGD also awards/deducts honour here (gain_honour/loose_honour); this
    * codebase has no honour/reputation system (confirmed missing, not yet
    * built), so that part is intentionally omitted rather than faked. */
   if( !IS_NPC( victim ) && !IS_NPC( ch )
   &&  IS_SET( victim->pcdata->flags, PCFLAG_SPARRING )
   &&  IS_SET( ch->pcdata->flags, PCFLAG_SPARRING )
   &&  victim->hit < 1 && victim != ch && init_hit >= 1 )
   {
      victim->hit = 1;
      stop_fighting( victim, TRUE );
      stop_fighting( ch, TRUE );
      send_to_char( "&RYou have fallen in combat!\r\n", victim );
      send_to_char( "&GYou are the victor in honorable combat!\r\n", ch );
   }
   ...
   switch ( victim->position )
   {
      ...
      default:
         if( dam > victim->max_hit / 4 )
         {
            ...
            /* v1.56: exempt immortals from mental_state loss here, matching
             * the same exemption gain_condition() already gives immortals
             * for hunger/thirst-driven mental_state loss. */
            if( !IS_IMMORTAL( victim ) && number_bits( 3 ) == 0 )
...

         {
            ...
            if( !IS_IMMORTAL( victim ) && number_bits( 2 ) == 0 )
               worsen_mental_state( victim, 1 );
         }
         break;
   }
   ...
   if( IS_NPC( victim ) )
   {
      if( !IS_NPC( ch ) )
      {
         for( x = 0; x < 32; x++ )
         {
            ...
            {
               if( !is_affected( ch, gsn_dipimmunity ) )
               {
...
OBJ_DATA *raw_kill( CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   /*
    * Take care of possessed mobs - detach the possessing player's live
    * connection BEFORE this NPC is extracted/freed, so their descriptor
    * never ends up pointing at freed memory. The original body is left
    * exactly where it was cast from; only the connection is reattached.
    */
   if( IS_NPC( victim ) && IS_AFFECTED( victim, AFF_POSSESS ) && victim->desc && victim->desc->original )
   {
      CHAR_DATA *original = victim->desc->original;

      send_to_char( "Your possession is violently severed as your host perishes!\r\n", original );

      affect_strip( victim, gsn_possess );
      REMOVE_BIT( victim->affected_by, AFF_POSSESS );

      victim->desc->character = original;
      victim->desc->original  = NULL;
      original->desc          = victim->desc;
      original->switched      = NULL;
      victim->desc            = NULL;
...
void dam_message( CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt )
{
   ...
   if( dt == TYPE_HIT || dam == 0 )
   {
      ...
      snprintf( buf2, 256, "You %s $N &W[&R%d&W]%c", vs, dam, punct );
      snprintf( buf3, 256, "$n %s you%c &W[&R%d&W]", vp, punct, dam );
   }
   else if( dt > TYPE_HIT && is_wielding_poisoned( ch ) )
   {
	 ...
	 attack = attack_table[0];
      ...
      snprintf( buf2, 256, "Your poisoned %s %s $N &W[&R%d&W]%c", attack, vp, dam, punct );
      snprintf( buf3, 256, "$n's poisoned %s %s you%c &W[&R%d&W]", attack, vp, punct, dam );
   }
   else
   {
      if( skill )
      ...
      snprintf( buf2, 256, "Your %s %s $N &W[&R%d&W]%c", attack, vp, dam, punct );
      snprintf( buf3, 256, "$n's %s %s you%c &W[&R%d&W]", attack, vp, punct, dam );
   }
