...
/*
 * v1.19, ported from SWGD misc.c do_sellhome but adapted to this
 * codebase's simpler single-room home model (do_buyhome above just
 * flips ROOM_EMPTY_HOME/ROOM_PLR_HOME on ch->plr_home directly - no
 * HOME_DATA linked list or per-home save file exists here, so none of
 * that infrastructure needed porting). Refund is half the 100000 buy
 * price, matching SWGD's 100000/50000 buy/sell ratio.
 */
void do_sell_home( CHAR_DATA * ch, const char *argument )
{
   ROOM_INDEX_DATA *room;
   ...
   {
      send_to_char( "&RThis room isn't owned.&w\r\n", ch );
      return;
   }
   ...
   send_to_char( "&GYou sell your home back for 50000 credits.\r\n&w", ch );
   do_save( ch, "" );
}
