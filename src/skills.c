...
bool check_skill( CHAR_DATA * ch, const char *command, const char *argument )
{
   ...
   if( !skill_table[sn]->skill_fun )
   {
      ...
      {
         int level = IS_NPC( ch ) ? ch->top_level : ch->skill_level[skill_table[sn]->guild];

         if( !IS_NPC( ch ) && skill_table[sn]->guild == FORCE_ABILITY && ch->subclass == SUBCLASS_PRODIGY )
            level += 50;
...
void learn_from_success( CHAR_DATA * ch, int sn )
{
   ...
   /* Blademaster gets raised adept cap for blade weapons and parry */
   if( !IS_NPC( ch ) && ch->subclass == SUBCLASS_BLADEMASTER
       && ( sn == gsn_lightsabers || sn == gsn_vibro_swords || sn == gsn_parry ) )
      adept = UMAX( adept, 150 );

   /* Sith Hunter gets an even higher cap on lightsabers and parry,
      reflecting GD's bonus for the same subclass */
   if( !IS_NPC( ch ) && ch->subclass == SUBCLASS_SITH_HUNTER
       && ( sn == gsn_lightsabers || sn == gsn_parry ) )
      adept = UMAX( adept, 200 );

   if( !IS_NPC( ch ) && ch->subclass == SUBCLASS_SITH_HUNTER
       && ( sn == gsn_lightsaber_crafting || sn == gsn_disarm ) )
      adept = UMAX( adept, 110 );

   /* v1.31: spray is ported now (do_spray, swskills.c) - Mercenary's SWGD
    * bonus can finally be wired in. */
...
/* v1.54: ported from SWGD. Called each combat round (violence_update,
 * update.c) to passively grow whichever fighting style is currently
 * active, capped at the character's Combat skill (plus Martial Artist
 * and Combat-class bonuses), same formula as do_style's maxstyle. */
...
void do_gouge( CHAR_DATA * ch, const char *argument )
{
   ...
   if( IS_NPC( ch ) || percent < ch->pcdata->learned[gsn_gouge] )
   {
      ...
      if( !IS_NPC( ch ) && ch->subclass == SUBCLASS_MARTIST )
         dam *= 2;   /* SWGD martist gouge bonus */
...
void do_steal( CHAR_DATA * ch, const char *argument )
{
   ...
   /* Situationally aware victims are much harder to steal from */
   ...
   if( victim->position == POS_FIGHTING || percent > ( IS_NPC( ch ) ? 90 : ch->pcdata->learned[gsn_steal] ) )
   {
      ...
      /* Mob becomes situationally aware for a duration after catching a thief */
      if( IS_NPC( victim ) && !IS_AFFECTED( victim, AFF_SIT_AWARE ) )
      {
         AFFECT_DATA af;
         af.type      = gsn_aware;
         ...
         affect_to_char( victim, &af );
         act( AT_ACTION, "$n looks around suspiciously.", victim, NULL, NULL, TO_ROOM );
      }
   ...
   if( !str_cmp( arg1, "credits" ) || !str_cmp( arg1, "credit" ) || !str_cmp( arg1, "money" ) )
   {
      ...
      if( ch->subclass == SUBCLASS_SNEAK )   /* SWGD subclass bonus -- 50%! */
...
void do_backstab( CHAR_DATA * ch, const char *argument )
{
   ...
   /* v1.54: ported from SWGD. Blocks an attacker who's already fighting
    * someone else from peeling off to backstab a fresh target - this
    * codebase only ever checked the victim's combat state, never the
    * attacker's, so an already-fighting player could still land a full
    * backstab multiplier against an unrelated third party. */
   ...
   /* Situationally aware victims can sense the approach */
   if( IS_AFFECTED( victim, AFF_SIT_AWARE ) && IS_AWAKE( victim ) )
   {
...
/* v1.54: ported from SWGD. Toggles PCFLAG_SPARRING - see the sparring
 * safety net in damage() (fight.c) for what this actually does in combat. */
void do_spar( CHAR_DATA * ch, const char *argument )
{
   if( IS_NPC( ch ) )
      return;
   ...
   if( argument[0] == '\0' )
   {
      send_to_char( "&RSyntax: spar <on/off>&w\r\n", ch );
      return;
   }

   if( !str_cmp( argument, "on" ) )
   {
      if( !IS_SET( ch->pcdata->flags, PCFLAG_SPARRING ) )
      {
         send_to_char( "Spar mode activated.\r\n", ch );
         SET_BIT( ch->pcdata->flags, PCFLAG_SPARRING );
      }
      else
         send_to_char( "Your sparring mode is already on.\r\n", ch );
      return;
   }
   ...
   if( !str_cmp( argument, "off" ) )
   {
      if( IS_SET( ch->pcdata->flags, PCFLAG_SPARRING ) )
      {
         send_to_char( "Spar mode deactivated.\r\n", ch );
         REMOVE_BIT( ch->pcdata->flags, PCFLAG_SPARRING );
      }
      else
         send_to_char( "Your sparring mode is already off.\r\n", ch );
      return;
   }
...
void trip( CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   if( victim->mount )
   {
      if( IS_AFFECTED( victim->mount, AFF_FLOATING ) )
         return;
...
void do_situational_awareness( CHAR_DATA * ch, const char *argument )
{
   AFFECT_DATA af;
   ...
   if( ch->pcdata->learned[gsn_aware] <= 0 )
   {
      send_to_char( "You lack the training to heighten your awareness.\r\n", ch );
      return;
   }

   if( IS_AFFECTED( ch, AFF_SIT_AWARE ) )
   {
      send_to_char( "You are already at heightened awareness.\r\n", ch );
      return;
   }

   if( number_percent(  ) > ch->pcdata->learned[gsn_aware] )
   {
      send_to_char( "You try to sharpen your focus but can't quite manage it.\r\n", ch );
      learn_from_failure( ch, gsn_aware );
      return;
   }
   ...
   af.location  = APPLY_AC;
   af.modifier  = -10;
   af.duration  = URANGE( 5, ch->pcdata->learned[gsn_aware] / 5, 30 );
   af.bitvector = AFF_SIT_AWARE;
   affect_to_char( ch, &af );

   send_to_char( "You sharpen your senses and heighten your awareness of your surroundings.\r\n", ch );
   act( AT_ACTION, "$n looks around carefully, alert to potential threats.", ch, NULL, NULL, TO_ROOM );
   learn_from_success( ch, gsn_aware );
}
...
void do_recall( CHAR_DATA * ch, const char *argument )
{
   ...
   /* if( IS_SET( ch->affected_by, AFF_CURSE ) )
   {
      ...
      return;
   } -- disabled along with spell_curse, see magic.c */
...
bool check_parry( CHAR_DATA * ch, CHAR_DATA * victim, int dt, int dam )
{
   ...
   /* Ranged attacks are blaster or bowcaster fire. Most characters cannot
      parry these at all; only certain Force-trained subclasses can. */
   is_ranged = ( dt == ( TYPE_HIT + WEAPON_BLASTER ) || dt == ( TYPE_HIT + WEAPON_BOWCASTER ) );
   ...
   {
      if( victim->subclass == SUBCLASS_SITH_HUNTER || victim->subclass == SUBCLASS_JEDI )
         can_parry_ranged = TRUE;

      if( is_ranged && !can_parry_ranged )
         return FALSE;

      /* Check main hand for lightsaber or vibro-sword */
      wield = get_eq_char( victim, WEAR_WIELD );
      if( wield != NULL && wield->item_type == ITEM_WEAPON
          && ( wield->value[3] == WEAPON_LIGHTSABER
            || wield->value[3] == WEAPON_VIBRO_SWORD ) )
         can_parry = TRUE;

      /* Check offhand if main hand failed */
      if( !can_parry )
      {
      ...
      /* Sliding scale base chance: scales with force and combat level.
         At 0/0 the multiplier is 0.5, at max it approaches 1.0.
         The cap then enforces the hard ceiling regardless of skill value. */
      force_lvl  = victim->skill_level[FORCE_ABILITY];
      combat_lvl = victim->skill_level[COMBAT_ABILITY];
      chances = (int)( victim->pcdata->learned[gsn_parry]
                * ( 0.5 + 0.5 * ( ( force_lvl + combat_lvl ) / 200.0 ) ) );

      /* Cap enforces the maximum parry rate:
         Blademaster: 90% ceiling, Sith Hunter: 75% ceiling, everyone else: 70% ceiling */
   ...
   /* Move drain — Blademaster is more efficient */
   if( !IS_NPC( victim ) )
   {
      if( victim->subclass == SUBCLASS_BLADEMASTER )
         victim->move -= UMAX( 0, 5 );
      else
         victim->move -= UMAX( 0, 15 );
      if( victim->move < 0 )
         victim->move = 0;
   }
   ...
   /*
    * Deflect - v1.18. Per SWGD help text: "In order for a blaster bolt to
    * be deflected, it must first be parried. Not all blaster bolts
    * parried will be deflected." This is a separate roll layered on top
    * of a successful ranged parry, not an independent check - and the
    * reflected bolt "loses some energy" and does less damage than the
    * original hit.
    */
   if( is_ranged && !IS_NPC( victim ) )
      check_deflect( ch, victim, dt, dam );
...
bool check_dodge( CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   if( number_range( 1, 100 ) > chances )
   {
      ...
      if( !IS_NPC( victim ) )
      {
         OBJ_DATA *jetpack = get_eq_char( victim, WEAR_ABOUT );

         if( jetpack && jetpack->item_type == ITEM_JETPACK && victim->pcdata->learned[gsn_jetpackdodge] > 0 )
         {
            double fraction = UMIN( 1.0, victim->skill_level[HUNTING_ABILITY] / 150.0 );
            int jchance = ( int )( victim->pcdata->learned[gsn_jetpackdodge] * ( 0.5 + 0.5 * fraction ) );

            jchance = UMIN( 60, jchance );

            if( number_range( 1, 100 ) <= jchance )
            {
               act( AT_SKILL, "$n's jetpack flares as $e rockets out of the way of your attack!", ch, NULL, victim, TO_CHAR );
               act( AT_SKILL, "Your jetpack flares as you rocket out of the way of $n's attack!", ch, NULL, victim, TO_VICT );
               learn_from_success( victim, gsn_jetpackdodge );
               return TRUE;
            }
            learn_from_failure( victim, gsn_jetpackdodge );
         }
      }
...
/*
 * Check for duck - ported from SWGD (v1.18). Smuggling-guild evasion skill;
 * unlike dodge/parry it costs stamina even on success (a full drop-and-roll
 * rather than a simple sidestep).
 */
bool check_duck( CHAR_DATA * ch, CHAR_DATA * victim )
{
   ...
   if( !IS_NPC( victim ) && victim->move <= 100 )
   {
      act( AT_SKILL, "You're too tired to duck out of $n's way!", ch, NULL, victim, TO_VICT );
      return FALSE;
   }

   if( !IS_NPC( victim ) )
      victim->move = UMAX( 0, victim->move - encumbrance( victim, 20 ) );
...
/*
 * Check for deflect - v1.18, called from within check_parry's ranged-parry
 * success path (not an independent check). Per SWGD help text: deflect
 * requires the shot to already be parried, isn't guaranteed even then,
 * and reflects reduced damage back at the attacker (the bolt "loses some
 * energy in the transformation"). Only lightsaber wielders can deflect -
 * check_parry's own ranged-parry branch also allows vibro-swords to block
 * (not reflect) blaster fire for Jedi/Sith Hunter, so this re-checks the
 * wielded weapon specifically.
 */
bool check_deflect( CHAR_DATA * ch, CHAR_DATA * victim, int dt, int dam )
{
   int chances;
   int force_lvl, combat_lvl, cap;
   OBJ_DATA *wield;
   ...
   wield = get_eq_char( victim, WEAR_WIELD );
   if( wield != NULL && wield->item_type == ITEM_WEAPON && wield->value[3] == WEAPON_LIGHTSABER )
      can_deflect = TRUE;
   ...
   force_lvl = victim->skill_level[FORCE_ABILITY];
   combat_lvl = victim->skill_level[COMBAT_ABILITY];
   chances = ( int )( victim->pcdata->learned[gsn_deflect] * ( 0.5 + 0.5 * ( ( force_lvl + combat_lvl ) / 200.0 ) ) );
   cap = ( victim->subclass == SUBCLASS_SITH_HUNTER ) ? 60 : 50;
   chances = URANGE( 5, chances, cap );
   ...
   victim->move = UMAX( 0, victim->move - 15 );

   if( !IS_SET( victim->pcdata->flags, PCFLAG_GAG ) )
      act( AT_SKILL, "You deflect $n's shot back with your lightsaber!", ch, NULL, victim, TO_VICT );

   if( !IS_NPC( ch ) && !IS_SET( ch->pcdata->flags, PCFLAG_GAG ) )
      act( AT_SKILL, "$N deflects your shot back with $S lightsaber!", ch, NULL, victim, TO_CHAR );
   else
      act( AT_SKILL, "$N deflects $n's shot back with $S lightsaber!", ch, NULL, victim, TO_NOTVICT );

   learn_from_success( victim, gsn_deflect );

   /* "This bolt does lose some energy in the transformation and so will
    * actually do less damage than if it were to hit its original target." */
   if( ch != victim && !char_died( ch ) )
      damage( victim, ch, ( int )( dam * 0.6 ), dt );
...
void do_brew( CHAR_DATA * ch, const char *argument )
{
   ...
   switch ( ch->substate )
   {
      default:
         ...
         if( !checktool )
         {
            send_to_char( "&RYou need a toolkit to prepare an elixir.\r\n", ch );
            return;
         }
         if( ( obj = get_obj_carry( ch, argument ) ) == NULL )
         {
         ...
         chance = IS_NPC( ch ) ? ch->top_level : ( int )( ch->pcdata->learned[gsn_brew] );
         if( number_percent(  ) < chance )
         {
            send_to_char( "&GYou begin the long process of preparing an elixir.\r\n", ch );
            act( AT_PLAIN, "$n begins preparing an elixir.", ch, NULL, argument, TO_ROOM );
            if( IS_IMMORTAL( ch ) )
               timer = 1;
               ...
               timer = 25;
            add_timer( ch, TIMER_DO_FUN, timer, do_brew, 1 );
            ch->dest_buf = strdup( arg );
            return;
         }
   ...
   level = IS_NPC( ch ) ? ch->skill_level[MEDICAL_ABILITY] : ( int )( ch->pcdata->learned[gsn_brew] );
   vnum = 10447;
   ...
   if( ( old = get_obj_carry( ch, arg ) ) == NULL )
   {
      send_to_char( "&RYou do not have that item.&w\r\n", ch );
      return;
   }
   if( old->item_type != ITEM_MEDICINE )
   {
   ...
   obj->weight = 1;
   STRFREE( obj->name );
   snprintf( buf, MAX_STRING_LENGTH, "an elixir made from a %s", old->short_descr );
   obj->name = STRALLOC( buf );
   STRFREE( obj->short_descr );
   obj->short_descr = STRALLOC( buf );
   STRFREE( obj->description );
   snprintf( buf, MAX_STRING_LENGTH, "An elixir made from a %s was left here.", old->short_descr );
   ...
   act( AT_PLAIN, "$n finishes preparing an elixir.", ch, NULL, argument, TO_ROOM );
...
void do_circle( CHAR_DATA * ch, const char *argument )
{
   ...
   /* Situationally aware victims are harder to circle */
...
void do_flamethrower( CHAR_DATA * ch, const char *argument )
{
   CHAR_DATA *victim;
   char arg[MAX_INPUT_LENGTH];
   OBJ_DATA *flamer;
   AFFECT_DATA af;
   int percent, dam, burn;
   ...
   if( arg[0] == '\0' )
   {
      ...
      {
         send_to_char( "Flamethrower whom?\r\n", ch );
         return;
      }
   }
   else if( ( victim = get_char_room( ch, arg ) ) == NULL )
   ...
   if( !is_legal_kill( ch, victim ) )
   {
      send_to_char( "You can't do that.\r\n", ch );
      return;
   }

   if( ( flamer = get_eq_char( ch, WEAR_WRIST_L ) ) == NULL || flamer->item_type != ITEM_FLAMETHROWER )
      flamer = get_eq_char( ch, WEAR_WRIST_R );

   if( !flamer || flamer->item_type != ITEM_FLAMETHROWER )
   {
      send_to_char( "You need to be wearing a wrist-flamethrower to do that.\r\n", ch );
      return;
   }

   percent = IS_NPC( ch ) ? ch->top_level : ch->pcdata->learned[gsn_flamethrower];
   percent += get_curr_dex( ch ) / 5;
   WAIT_STATE( ch, skill_table[gsn_flamethrower]->beats );

   if( number_percent(  ) < percent )
   {
      dam = UMAX( 5, number_range( flamer->value[1], flamer->value[2] ) );

      act( AT_FIRE, "You douse $N in flame from your wrist-flamethrower!", ch, NULL, victim, TO_CHAR );
      act( AT_FIRE, "$n douses you in flame from $s wrist-flamethrower!", ch, NULL, victim, TO_VICT );
      act( AT_FIRE, "$n douses $N in flame from $s wrist-flamethrower!", ch, NULL, victim, TO_NOTVICT );

      damage( ch, victim, dam, gsn_flamethrower );

      if( char_died( victim ) )
      {
         learn_from_success( ch, gsn_flamethrower );
         return;
      }

      burn = UMAX( 1, ( int )( dam * 0.25 ) );
      af.type = gsn_flamethrower;
      af.duration = number_range( 2, 4 );
      af.location = APPLY_NONE;
      af.modifier = burn;
      af.bitvector = AFF_ONFIRE;
      affect_to_char( victim, &af );
      act( AT_FIRE, "$N is set ablaze!", ch, NULL, victim, TO_CHAR );
      act( AT_FIRE, "You are set ablaze!", ch, NULL, victim, TO_VICT );

      learn_from_success( ch, gsn_flamethrower );
   }
   else
   {
      learn_from_failure( ch, gsn_flamethrower );
      act( AT_SKILL, "You squeeze the trigger but only a sputter of flame comes out.", ch, NULL, victim, TO_CHAR );
      act( AT_SKILL, "$n squeezes the trigger of $s wrist-flamethrower, but only a sputter comes out.", ch, NULL,
           victim, TO_VICT );
      damage( ch, victim, 0, gsn_flamethrower );
   }
   return;
}

void do_concussion_rocket( CHAR_DATA * ch, const char *argument )
{
   CHAR_DATA *victim;
   char arg[MAX_INPUT_LENGTH];
   OBJ_DATA *jetpack;
   int percent, i, hits, misses, hp_dam, mana_dam;
   ...
   if( arg[0] == '\0' )
   {
      ...
      {
         send_to_char( "Fire concussion rockets at whom?\r\n", ch );
         return;
      }
   }
   else if( ( victim = get_char_room( ch, arg ) ) == NULL )
   ...
   if( ( jetpack = get_eq_char( ch, WEAR_ABOUT ) ) == NULL || jetpack->item_type != ITEM_JETPACK )
   {
      send_to_char( "You need to be wearing a jetpack with rockets attached to do that.\r\n", ch );
      return;
   }

   percent = IS_NPC( ch ) ? ch->top_level : ch->pcdata->learned[gsn_concussion_rocket];
   WAIT_STATE( ch, skill_table[gsn_concussion_rocket]->beats );

   if( number_percent(  ) < percent )
   {
      act( AT_FIRE, "You trigger the concussion rockets mounted on your jetpack!", ch, NULL, victim, TO_CHAR );
      act( AT_FIRE, "$n triggers the concussion rockets mounted on $s jetpack!", ch, NULL, victim, TO_VICT );
      act( AT_FIRE, "$n triggers the concussion rockets mounted on $s jetpack, aimed at $N!", ch, NULL, victim,
           TO_NOTVICT );
      ...
      for( i = 0; i < 3; i++ )
      {
         ...
         hits++;
         hp_dam = number_range( UMAX( 1, ch->skill_level[HUNTING_ABILITY] / 6 ), UMAX( 1, ch->skill_level[HUNTING_ABILITY] / 3 ) );
         mana_dam = number_range( ch->skill_level[HUNTING_ABILITY], ch->skill_level[HUNTING_ABILITY] * 2 );

         damage( ch, victim, hp_dam, gsn_concussion_rocket );
         if( char_died( victim ) )
            break;

         victim->mana = URANGE( 0, victim->mana - mana_dam, victim->max_mana );
      }

      if( misses > 0 && hits > 0 )
         ch_printf( ch, "%d of your 3 rockets miss their mark.\r\n", misses );
      else if( misses == 3 )
         send_to_char( "All 3 of your rockets miss their mark.\r\n", ch );

      if( hits > 0 && !char_died( victim ) )
         act( AT_SKILL, "$n's connection to the Force feels distant.", ch, NULL, victim, TO_VICT );

      learn_from_success( ch, gsn_concussion_rocket );
   }
   else
   {
      learn_from_failure( ch, gsn_concussion_rocket );
      act( AT_SKILL, "You trigger the concussion rockets but the launch sequence fails.", ch, NULL, victim, TO_CHAR );
      act( AT_SKILL, "$n triggers $s jetpack's rockets, but the launch sequence fails.", ch, NULL, victim, TO_VICT );
      damage( ch, victim, 0, gsn_concussion_rocket );
   }
   return;
}
...
void do_slice( CHAR_DATA * ch, const char *argument )
{
   ...
   {
      send_to_char( "You need to be wielding a lightsaber or vibro-sword to slice.\r\n", ch );
      return;
   }

   if( obj->item_type != ITEM_WEAPON )
   {
      send_to_char( "You can only slice with a lightsaber or vibro-sword.\r\n", ch );
      return;
   }

   wtype = obj->value[3];
   if( wtype != WEAPON_LIGHTSABER && wtype != WEAPON_VIBRO_SWORD )
   {
      send_to_char( "You can only slice with a lightsaber or vibro-sword.\r\n", ch );
      return;
   }

   if( !IS_NPC( victim ) && !is_legal_kill( ch, victim ) )
   {
      send_to_char( "You can't do that.\r\n", ch );
      return;
   }

   percent = IS_NPC( ch ) ? 80 : ch->pcdata->learned[gsn_slice];
   ...
   else
   {
      ...
      act( AT_SKILL, "You attempt to slice $N but can't find an opening.", ch, NULL, victim, TO_CHAR );
      act( AT_SKILL, "$n attempts to slice you but fails to connect.", ch, NULL, victim, TO_VICT );
      global_retcode = damage( ch, victim, 0, gsn_slice );
   }
   return;
}
