...
void save_ranks( CLAN_DATA * clan )
{
   FILE *fp;
   char filename[256];
   int x;

   if( !clan || !clan->filename || clan->filename[0] == '\0' )
      return;

   snprintf( filename, 256, "%s%s.rank", CLAN_DIR, clan->filename );
   if( ( fp = fopen( filename, "w" ) ) == NULL )
   {
      bug( "%s: fopen %s", __func__, filename );
      return;
   }
   for( x = 0; x <= clan->top_rank; x++ )
   {
      if( !clan->ranks[x] ) continue;
      fprintf( fp, "#RANK\n" );
      fprintf( fp, "Name      %s~\n", clan->ranks[x]->name );
      fprintf( fp, "Slot      %d\n",  clan->ranks[x]->slot );
      fprintf( fp, "Authority %d\n",  clan->ranks[x]->authority );
      fprintf( fp, "Wage      %d\n",  clan->ranks[x]->wage );
      fprintf( fp, "End\n\n" );
   }
   fprintf( fp, "#END\n" );
...
void load_ranks( CLAN_DATA * clan )
{
   FILE *fp;
   char filename[256];
   const char *word;
   ...
   for( ; ; )
   {
      ...
      if( !str_cmp( word, "#RANK" ) )
      {
         ...
         if( clan->top_rank >= MAX_CLANRANK - 1 )
         {
            bug( "%s: too many ranks in %s", __func__, filename );
            fread_to_eol( fp );
            continue;
         }
         sn = ++clan->top_rank;
         CREATE( rank, RANK_DATA, 1 );
         rank->clan      = clan;
         rank->name      = STRALLOC( "Unnamed" );
         rank->slot      = slot++;
         rank->authority = sn + 1;
         rank->wage      = 0;
         clan->ranks[sn] = rank;
         ...
         for( ; ; )
         {
            ...
            switch( UPPER( word[0] ) )
            {
               case 'A': KEY( "Authority", rank->authority, fread_number( fp ) ); break;
               case 'E':
                  if( !str_cmp( word, "End" ) ) { fMatch = TRUE; goto rank_done; }
                  break;
               case 'N':
                  ...
                  break;
               case 'S': KEY( "Slot", rank->slot, fread_number( fp ) ); break;
               case 'W': KEY( "Wage", rank->wage, fread_number( fp ) ); break;
            }
            if( !fMatch )
...
void save_roster( CLAN_DATA * clan )
{
   FILE *fp;
   char filename[256];
   ROSTER_DATA *roster;
   ...
   snprintf( filename, 256, "%s%s.roster", CLAN_DIR, clan->filename );
   if( ( fp = fopen( filename, "w" ) ) == NULL )
   {
      bug( "%s: fopen %s", __func__, filename );
      return;
   }
   for( roster = clan->first_roster; roster; roster = roster->next_roster )
   {
      fprintf( fp, "#ROSTER\n" );
      fprintf( fp, "Name     %s~\n", roster->name );
      fprintf( fp, "Rank     %s~\n", roster->rank_name ? roster->rank_name : "" );
      fprintf( fp, "End\n\n" );
   }
   fprintf( fp, "#END\n" );
   FCLOSE( fp );
...
void load_roster( CLAN_DATA * clan )
{
   FILE *fp;
   char filename[256];
   const char *word;
   ...
   for( ; ; )
   {
      ...
      if( !str_cmp( word, "#END" ) )
         break;
      if( !str_cmp( word, "#ROSTER" ) )
      {
         ROSTER_DATA *roster;
         CREATE( roster, ROSTER_DATA, 1 );
         roster->clan      = clan;
         roster->name      = STRALLOC( "" );
         roster->rank_name = STRALLOC( "" );
         ...
         for( ; ; )
         {
            ...
            switch( UPPER( word[0] ) )
            {
               case 'E':
                  if( !str_cmp( word, "End" ) )
                  {
                     LINK( roster, clan->first_roster, clan->last_roster, next_roster, prev_roster );
                     fMatch = TRUE;
                     goto roster_done;
                  }
                  break;
               ...
               case 'R':
                  if( !str_cmp( word, "Rank" ) )
                  {
                     STRFREE( roster->rank_name );
                     roster->rank_name = fread_string( fp );
                     fMatch = TRUE;
                  }
                  break;
            }
...
/* Add a new blank rank to a clan, return its sn or -1 if full */
int add_rank( CLAN_DATA * clan )
{
   RANK_DATA *rank;
   int sn;
...
/* Find rank by name, return sn or -1 */
int get_rank_by_name( CLAN_DATA * clan, const char *name )
{
   int x;

   for( x = 0; x <= clan->top_rank; x++ )
      if( clan->ranks[x] && !str_cmp( clan->ranks[x]->name, name ) )
         return x;
   return -1;
}

/* Find roster entry by player name */
ROSTER_DATA *get_roster_by_name( CLAN_DATA * clan, const char *name )
{
   ROSTER_DATA *roster;
...
/* Get wages owed to a member based on their clan_rank string */
int get_wages_for_member( CLAN_DATA * clan, CHAR_DATA * ch )
{
   int sn;

   if( !clan || !ch->pcdata->clan_rank || ch->pcdata->clan_rank[0] == '\0' )
      return 0;
   sn = get_rank_by_name( clan, ch->pcdata->clan_rank );
   if( sn == -1 )
      return 0;
   return clan->ranks[sn]->wage;
}

/* Called on induct (action=1) or resign/outcast (action=0) */
void update_roster( CHAR_DATA * ch, int action )
{
   CLAN_DATA *clan;
   ROSTER_DATA *roster;
   ...
   if( action == 1 )   /* add */
   {
      if( get_roster_by_name( clan, ch->name ) )
         return;   /* already on roster */

      CREATE( roster, ROSTER_DATA, 1 );
      roster->clan      = clan;
      roster->name      = STRALLOC( ch->name );
      roster->rank_name = STRALLOC(
         ch->pcdata->clan_rank && ch->pcdata->clan_rank[0] != '\0'
         ? ch->pcdata->clan_rank : "" );
      LINK( roster, clan->first_roster, clan->last_roster, next_roster, prev_roster );
      save_roster( clan );
   }
   ...
   {
      roster = get_roster_by_name( clan, ch->name );
      if( !roster ) return;
      UNLINK( roster, clan->first_roster, clan->last_roster, next_roster, prev_roster );
      STRFREE( roster->name );
      STRFREE( roster->rank_name );
      DISPOSE( roster );
      save_roster( clan );
...
void do_ranks( CHAR_DATA * ch, const char *argument )
{
   CLAN_DATA *clan;
   int x;
   ...
   else
   {
      if( !ch->pcdata->clan )
      {
         send_to_char( "You aren't part of a clan.\r\n", ch );
         return;
      }
      clan = ch->pcdata->clan;
   }
   ...
   if( clan->top_rank < 0 )
   {
      send_to_char( "This clan has no ranks defined yet.\r\n", ch );
      return;
   }

   set_char_color( AT_CYAN, ch );
   ch_printf( ch, "\r\n%-25s  %-9s  %s\r\n", "Rank Name", "Authority", "Wage" );
   send_to_char( "---------------------------------------------------\r\n", ch );
   for( x = 0; x <= clan->top_rank; x++ )
   {
      if( !clan->ranks[x] ) continue;
      ch_printf( ch, "%-25s  %-9d  %d\r\n",
                 clan->ranks[x]->name,
                 clan->ranks[x]->authority,
                 clan->ranks[x]->wage );
   }
...
void do_roster( CHAR_DATA * ch, const char *argument )
{
   ...
   else
   {
      ...
      /* Only leader, officers, or bestowed members can view roster */
      if( str_cmp( ch->name, clan->leader )
          && str_cmp( ch->name, clan->number1 )
          && str_cmp( ch->name, clan->number2 )
          && !( ch->pcdata->bestowments && is_name( "roster", ch->pcdata->bestowments ) ) )
      {
         send_to_char( "You aren't privileged enough to view the roster.\r\n", ch );
         return;
      }
   }
   ...
   for( roster = clan->first_roster; roster; roster = roster->next_roster )
   {
      ch_printf( ch, "%-30s  %s\r\n",
                 roster->name,
                 roster->rank_name && roster->rank_name[0] != '\0'
                 ? roster->rank_name : "(none)" );
      count++;
   }
   ch_printf( ch, "\r\n%d member%s on roster.\r\n", count, count == 1 ? "" : "s" );
   set_char_color( AT_WHITE, ch );
}
...
void do_setrank( CHAR_DATA * ch, const char *argument )
{
   ...
   if( str_cmp( ch->name, clan->leader ) && !IS_IMMORTAL( ch ) )
   {
      send_to_char( "Only your leader can manage ranks.\r\n", ch );
      return;
   }
   ...
   if( !str_cmp( arg1, "add" ) )
   {
      if( argument[0] == '\0' )
      {
         send_to_char( "Syntax: setrank add <name>\r\n", ch );
         return;
      }
      if( get_rank_by_name( clan, argument ) != -1 )
      {
         send_to_char( "A rank with that name already exists.\r\n", ch );
         return;
      }
      sn = add_rank( clan );
      if( sn == -1 )
      {
         send_to_char( "Maximum number of ranks reached.\r\n", ch );
         return;
      }
      STRFREE( clan->ranks[sn]->name );
      clan->ranks[sn]->name = STRALLOC( argument );
      save_ranks( clan );
      send_to_char( "Rank added.\r\n", ch );
      return;
   }

   if( !str_cmp( arg1, "del" ) || !str_cmp( arg1, "delete" ) )
   {
      RANK_DATA *rank;
      int x;
      ...
      if( argument[0] == '\0' )
      {
         send_to_char( "Syntax: setrank del <name>\r\n", ch );
         return;
      }
      sn = get_rank_by_name( clan, argument );
      if( sn == -1 )
      {
         send_to_char( "No rank with that name.\r\n", ch );
         return;
      }
      rank = clan->ranks[sn];
      STRFREE( rank->name );
      DISPOSE( rank );
      /* compact the array */
      for( x = sn; x < clan->top_rank; x++ )
         clan->ranks[x] = clan->ranks[x + 1];
      clan->ranks[clan->top_rank] = NULL;
      clan->top_rank--;
      save_ranks( clan );
      send_to_char( "Rank deleted.\r\n", ch );
      return;
   }
   ...
   if( arg2[0] == '\0' )
   {
      send_to_char( "Syntax: setrank add <name>\r\n", ch );
      send_to_char( "Syntax: setrank del <name>\r\n", ch );
      send_to_char( "Syntax: setrank <rank> name <newname>\r\n", ch );
      send_to_char( "Syntax: setrank <rank> authority <1-20>\r\n", ch );
      send_to_char( "Syntax: setrank <rank> wage <credits>\r\n", ch );
      return;
   }

   if( ( sn = get_rank_by_name( clan, arg1 ) ) == -1 )
   {
      send_to_char( "No rank with that name.\r\n", ch );
      return;
   }
   ...
   {
      if( argument[0] == '\0' )
      { send_to_char( "New name cannot be empty.\r\n", ch ); return; }
      if( get_rank_by_name( clan, argument ) != -1 )
   ...
   if( !str_cmp( arg2, "authority" ) )
   {
      int val = atoi( argument );
      if( val < 1 || val > 20 )
      { send_to_char( "Authority must be 1-20 (1=highest).\r\n", ch ); return; }
      clan->ranks[sn]->authority = val;
      save_ranks( clan );
      send_to_char( "Done.\r\n", ch );
      return;
   }

   if( !str_cmp( arg2, "wage" ) )
   {
      int val = atoi( argument );
      if( val < 0 )
      { send_to_char( "Wage cannot be negative.\r\n", ch ); return; }
      if( val > 1000000 )
      { send_to_char( "Wage must be under 1,000,000.\r\n", ch ); return; }
      clan->ranks[sn]->wage = val;
      save_ranks( clan );
      send_to_char( "Done.\r\n", ch );
      return;
   }

   send_to_char( "Unknown field. Use: name, authority, wage.\r\n", ch );
}

/* Immortal utility: recount all clan members from roster */
void do_fix_clan_membership( CHAR_DATA * ch, const char *argument )
{
   CLAN_DATA *clan;
   ROSTER_DATA *roster;
   int count;
...
/* ---- Assign a clan rank to a player ---- */
/* Syntax: clanrank <player> <rank>  (leader only, or imm) */
void do_clanrank( CHAR_DATA * ch, const char *argument )
{
   char arg1[MAX_INPUT_LENGTH];
   CHAR_DATA *victim;
   CLAN_DATA *clan;
   ...
   {
      send_to_char( "Syntax: clanrank <player> <rank>\r\n", ch );
      send_to_char( "        clanrank <player> none     (to clear rank)\r\n", ch );
      return;
   }
   ...
   if( !IS_IMMORTAL( ch )
       ...
       && str_cmp( ch->name, clan->number1 ) )
   {
      send_to_char( "Only the clan leader or first officer can assign ranks.\r\n", ch );
      return;
   }

   if( !str_cmp( argument, "none" ) )
   {
      STRFREE( victim->pcdata->clan_rank );
      victim->pcdata->clan_rank = STRALLOC( "" );
      /* Update roster entry */
      roster = get_roster_by_name( clan, victim->name );
      if( roster )
      {
         STRFREE( roster->rank_name );
         roster->rank_name = STRALLOC( "" );
      ...
      send_to_char( "Rank cleared.\r\n", ch );
      act( AT_CYAN, "Your clan rank has been cleared.", victim, NULL, NULL, TO_CHAR );
   ...
   if( sn == -1 )
   {
      send_to_char( "No such rank in this clan. Use 'ranks' to see the list.\r\n", ch );
      return;
   }

   STRFREE( victim->pcdata->clan_rank );
   victim->pcdata->clan_rank = QUICKLINK( clan->ranks[sn]->name );
   ...
   {
      STRFREE( roster->rank_name );
      roster->rank_name = QUICKLINK( clan->ranks[sn]->name );
      save_roster( clan );
   }

   ch_printf( ch, "%s has been assigned rank: %s\r\n", victim->name, clan->ranks[sn]->name );
   ch_printf( victim, "You have been assigned the rank of %s.\r\n", clan->ranks[sn]->name );
   save_char_obj( victim );
}
   ...
   else
   {
            ...
            fprintf( fp, "Area         %s~\n", pArea->filename );
      {
         ...
         for( x = CARGO_NONE + 1; x <= top_cargo_sn; x++ )
            fprintf( fp, "Resource     %d %d %d %d %d\n", x, planet->rcost[x], planet->resource[x],
                     planet->consumes[x], planet->produces[x] );
      }
      fprintf( fp, "End\n\n" );
      fprintf( fp, "#END\n" );
      FCLOSE( fp );
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'R':
            if( !str_cmp( word, "Resource" ) )
            {
               ...
               sscanf( line, "%d %d %d %d %d", &x0, &x1, &x2, &x3, &x4 );
               if( x0 > 0 && x0 < MAX_CARGO )
               {
                  planet->rcost[x0] = x1;
                  planet->resource[x0] = x2;
                  planet->consumes[x0] = x3;
                  planet->produces[x0] = x4;
               }
               fMatch = TRUE;
            }
            break;
...
void load_clans()
{
   ...
   /* Load rank and roster data for each clan */
   for( clan = first_clan; clan; clan = clan->next )
   {
      clan->top_rank    = -1;
      clan->first_roster = NULL;
      clan->last_roster  = NULL;
      load_ranks( clan );
      load_roster( clan );
   }

   log_string( " Done ranks and rosters" );
}
...
void do_setplanet( CHAR_DATA * ch, const char *argument )
{
   ...
   if( arg1[0] == '\0' )
   {
      ...
      send_to_char( " resource produces consumes\r\n", ch );
      return;
   }
   ...
   if( !strcmp( arg2, "resource" ) )
   {
      ...
      cargonum = get_cargo_num( arg3 );
      if( cargonum == CARGO_NONE && is_number( arg3 ) )
         cargonum = atoi( arg3 );

      if( cargonum != CARGO_NONE && cargonum <= top_cargo_sn )
      {
         planet->resource[cargonum] = atoi( argument );
         send_to_char( "Done.\r\n", ch );
   ...
   if( !strcmp( arg2, "produces" ) )
   {
      ...
      if( cargonum != CARGO_NONE && cargonum <= top_cargo_sn )
      {
         planet->produces[cargonum] = atoi( argument );
         send_to_char( "Done.\r\n", ch );
         save_planet( planet );
         return;
      }
   ...
   if( !strcmp( arg2, "consumes" ) )
   {
      ...
      if( cargonum != CARGO_NONE && cargonum <= top_cargo_sn )
      {
         planet->consumes[cargonum] = atoi( argument );
         send_to_char( "Done.\r\n", ch );
         update_cargo_costs( planet );
         save_planet( planet );
         return;
...
void do_resign( CHAR_DATA * ch, const char *argument )
{
   ...
   update_roster( ch, 0 );
   STRFREE( ch->pcdata->clan_rank );
   ch->pcdata->clan_rank = STRALLOC( "" );
...
void do_myclan( CHAR_DATA * ch, const char *argument )
{
   ...
   ch_printf( ch, "%-20s: %s\r\n",
              ...
              clan->clan_type == CLAN_SUBCLAN ? "Sub-clan" : "Organization",
              clan->name );
   ch_printf( ch, "Description : %s\r\n", clan->description ? clan->description : "(none)" );
   ch_printf( ch, "Leader      : %-12s\r\n", clan->leader ? clan->leader : "(none)" );
   if( clan->number1 && clan->number1[0] != '\0' )
      ch_printf( ch, "First Off.  : %-12s\r\n", clan->number1 );
   if( clan->number2 && clan->number2[0] != '\0' )
      ch_printf( ch, "Second Off. : %-12s\r\n", clan->number2 );
   ch_printf( ch, "Pkills: %d  Pdeaths: %d\r\n", clan->pkills, clan->pdeaths );
...
void do_overthrow( CHAR_DATA * ch, const char *argument )
{
   ...
   {
      send_to_char( "You can't overthrow a clan that already has a leader!\r\n", ch );
      return;
   }
   ...
   clan->members++;
   send_to_char( "You have taken control of the clan!\r\n", ch );
   snprintf( buf, MAX_STRING_LENGTH, "%s has been overthrown by %s!", clan->name, ch->name );
   echo_to_all( AT_RED, buf, 0 );
   save_clan( clan );
   do_save( ch, "" );
}
...
void do_abdicate( CHAR_DATA * ch, const char *argument )
{
   ...
   if( str_cmp( ch->name, clan->leader ) )
   {
      send_to_char( "Only the clan leader can abdicate.\r\n", ch );
      return;
   }

   /* Transfer to number1 if available, else number2, else leave empty */
   STRFREE( clan->leader );
   if( clan->number1 && clan->number1[0] != '\0' )
   {
      ...
      STRFREE( clan->number1 );
      clan->number1 = STRALLOC( "" );
      ch_printf( ch, "You abdicate leadership. %s now leads %s.\r\n",
                 clan->leader, clan->name );
      snprintf( buf, MAX_STRING_LENGTH, "%s has abdicated leadership of %s. %s now leads.",
                ch->name, clan->name, clan->leader );
   }
   else if( clan->number2 && clan->number2[0] != '\0' )
   {
   ...
   {
      clan->leader = STRALLOC( "" );
      send_to_char( "You abdicate leadership. The clan is now leaderless.\r\n", ch );
      snprintf( buf, MAX_STRING_LENGTH, "%s has abdicated leadership of %s. The clan is now leaderless.",
                ch->name, clan->name );
   }

   echo_to_all( AT_CYAN, buf, 0 );
   save_clan( clan );
   do_save( ch, "" );
}
