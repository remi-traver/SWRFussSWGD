...
bool saves_poison_death( int level, CHAR_DATA * victim )
{
   ...
   if( victim->subclass == SUBCLASS_SURVIVOR || victim->subclass == SUBCLASS_FORCE_SENSITIVE )
      save += 5;                /* SWGD: 2-in-20 floor for the survival subs */
...
void do_cast( CHAR_DATA * ch, const char *argument )
{
   ...
   if( !IS_NPC( ch ) && ch->subclass != SUBCLASS_FALSE_PROPHET && abs( ch->alignment - skill->alignment ) > 1010 )
   {
   ...
   else
   {
      ...
      {
         int force_level = ch->skill_level[FORCE_ABILITY];

         if( !IS_NPC( ch ) && ch->subclass == SUBCLASS_PRODIGY )
            force_level += 75;
         if( !IS_NPC( ch ) && ch->subclass == SUBCLASS_FALSE_PROPHET )
            force_level -= 25;

         start_timer( &time_used );
         retcode = ( *skill->spell_fun ) ( sn, force_level, ch, vo );
         end_timer( &time_used );
...
/* Disabled - had no skills.dat registration and was never actually
 * castable in-game; kept here commented out rather than deleted.
...
ch_ret spell_dark_frenzy( int sn, int level, CHAR_DATA * ch, void *vo )
{
   AFFECT_DATA af;
   double fraction;
   int combined, bonus, duration;

   if( ch->alignment >= 0 )
   {
      send_to_char( "You lack the darkness within to fuel a frenzy.\r\n", ch );
      return rSPELL_FAILED;
   }

   if( !ch->fighting )
   {
      send_to_char( "You can only do that while fighting.\r\n", ch );
      return rSPELL_FAILED;
   }

   if( IS_AFFECTED( ch, AFF_DARK_FRENZY ) )
   {
      send_to_char( "You are already in a frenzy!\r\n", ch );
      return rSPELL_FAILED;
   }

   combined = ch->skill_level[COMBAT_ABILITY] + ch->skill_level[FORCE_ABILITY];
   fraction = UMIN( 1.0, combined / 300.0 );
   bonus = 1 + ( int )( fraction * 7 );
   duration = 1 + ( int )( fraction * 4 );

   af.type = sn;
   af.duration = duration;
   af.location = APPLY_NONE;
   af.modifier = bonus;
   af.bitvector = AFF_DARK_FRENZY;
   affect_to_char( ch, &af );

   set_char_color( AT_RED, ch );
   send_to_char( "Rage consumes you as you give in to the dark side!\r\n", ch );
   act( AT_RED, "$n's eyes burn with dark side fury!", ch, NULL, NULL, TO_ROOM );
...
ch_ret spell_black_lightning( int sn, int level, CHAR_DATA * ch, void *vo )
{
   CHAR_DATA *victim = ( CHAR_DATA * ) vo;
   OBJ_DATA *wield;
   double fraction, reflect_chance, bonus_ceiling;
   int dam;
   ...
   /* Lightsaber reflect check - PCs only, before the save */
   wield = get_eq_char( victim, WEAR_WIELD );
   if( !IS_NPC( victim ) && wield != NULL && wield->item_type == ITEM_WEAPON
       && wield->value[3] == WEAPON_LIGHTSABER )
   {
      bonus_ceiling = 20.0;
      if( victim->pcdata->learned[gsn_lightsabers] > 100 )
         bonus_ceiling += ( victim->pcdata->learned[gsn_lightsabers] - 100 ) * 0.2;

      fraction = URANGE( 0.0, ( victim->skill_level[FORCE_ABILITY] - 50 ) / 100.0, 1.0 );
      reflect_chance = fraction * bonus_ceiling;

      if( ( double )number_percent(  ) <= reflect_chance )
      {
         act( AT_BLUE, "$N catches the lightning on $S lightsaber and hurls it back at you!", ch, NULL, victim, TO_CHAR );
         act( AT_BLUE, "You catch the lightning on your lightsaber and hurl it back at $n!", ch, NULL, victim, TO_VICT );
         act( AT_BLUE, "$N catches the bolt on $S blade and redirects it at $n!", ch, NULL, victim, TO_NOTVICT );
         damage( victim, ch, dam, sn );
         return char_died( ch ) ? rCHAR_DIED : rNONE;
      }
   }

   if( saves_poison_death( level + 15, victim ) )
      return damage( ch, victim, dam, sn );
   else
   {
...
/* v1.55: Chain Lightning - new spell (not an SWGD port), requested as
 * "force lightning that hits everyone in the room, including other
 * players." Reuses force lightning's per-target damage, lightsaber
 * reflect check, and poison-death save (spell_black_lightning above),
 * but strikes every legal target in the room once each instead of
 * repeatedly channeling into a single victim. Room-iteration eligibility
 * (group/legality/visibility/safe-room checks) mirrors do_hitall
 * (skills.c), the existing room-wide-attack pattern in this codebase, so
 * it respects the same PK safety rules everything else does. */
ch_ret spell_chain_lightning( int sn, int level, CHAR_DATA * ch, void *vo )
{
   ...
   set_char_color( AT_MAGIC, ch );
   send_to_char( "Arcs of lightning leap from your fingertips to every foe in the room!\r\n", ch );
   act( AT_MAGIC, "Arcs of lightning leap from $n's fingertips, chaining between $s foes!", ch, NULL, NULL, TO_ROOM );
   ...
   for( vch = ch->in_room->first_person; vch; vch = vch_next )
   {
      ...
      act( AT_BLUE, "Bolts of electricity leap onto $N, sending $M into a fit of painful spasms.", ch, NULL, vch,
           TO_NOTVICT );
      act( AT_BLUE, "Bolts of electricity leap onto $N, sending $M into a fit of painful spasms.", ch, NULL, vch,
           TO_CHAR );
      act( AT_BLUE, "Intense pain spreads through your body as chained lightning from $N assaults you.", vch, NULL, ch,
           TO_CHAR );

      /* Lightsaber reflect check - PCs only, before the save - same as
       * spell_black_lightning above. */
      wield = get_eq_char( vch, WEAR_WIELD );
      if( !IS_NPC( vch ) && wield != NULL && wield->item_type == ITEM_WEAPON && wield->value[3] == WEAPON_LIGHTSABER )
      {
         bonus_ceiling = 20.0;
         if( vch->pcdata->learned[gsn_lightsabers] > 100 )
            bonus_ceiling += ( vch->pcdata->learned[gsn_lightsabers] - 100 ) * 0.2;

         fraction = URANGE( 0.0, ( vch->skill_level[FORCE_ABILITY] - 50 ) / 100.0, 1.0 );
         reflect_chance = fraction * bonus_ceiling;

         if( ( double )number_percent(  ) <= reflect_chance )
         {
            act( AT_BLUE, "$N catches the lightning on $S lightsaber and hurls it back at you!", ch, NULL, vch, TO_CHAR );
            act( AT_BLUE, "You catch the lightning on your lightsaber and hurl it back at $n!", ch, NULL, vch, TO_VICT );
            act( AT_BLUE, "$N catches the bolt on $S blade and redirects it at $n!", ch, NULL, vch, TO_NOTVICT );
            damage( vch, ch, dam, sn );
            if( char_died( ch ) )
               return rCHAR_DIED;
            continue;
         }
      ...
      if( saves_poison_death( level + 15, vch ) )
         damage( ch, vch, dam / 2, sn );
      else
