...
void show_visible_affects_to_char( CHAR_DATA * victim, CHAR_DATA * ch )
{
   ...
   if( !IS_IMMORTAL( ch ) && IS_AFFECTED( victim, AFF_MASKED_AURA ) )
      return;   /* maskaura hides all visible aura descriptions - v1.18 */
   ...
   /* if( IS_AFFECTED( victim, AFF_FIRESHIELD ) )
   {
      ...
      ch_printf( ch, "%s is engulfed within a blaze of mystical flame.\r\n",
                 IS_NPC( victim ) ? capitalize( victim->short_descr ) : ( victim->name ) );
   } -- disabled, AFF_FIRESHIELD reclaimed for AFF_DARK_FRENZY, see magic.c */
...
/*Scryn 8/13*/
   /* if( IS_AFFECTED( victim, AFF_ICESHIELD ) )
   {
      ...
      ch_printf( ch, "%s is ensphered by shards of glistening ice.\r\n",
                 IS_NPC( victim ) ? capitalize( victim->short_descr ) : ( victim->name ) );
   } -- disabled, AFF_ICESHIELD reclaimed for AFF_DARK_FATIGUE, see magic.c */
...
void show_char_to_char_0( CHAR_DATA * victim, CHAR_DATA * ch )
{
   ...
   if( IS_IMMORTAL( ch ) || !IS_AFFECTED( victim, AFF_MASKED_AURA ) )
   {
      ...
      /* if( IS_AFFECTED( victim, AFF_FAERIE_FIRE ) )
         strlcat( buf, "&P(Pink Aura)&w ", MAX_STRING_LENGTH ); -- disabled, see magic.c */
...
void do_look( CHAR_DATA * ch, const char *argument )
{
   ...
   if( ( pexit = find_door( ch, arg1, TRUE ) ) != NULL )
   {
       ...
       */
      if( pexit->to_room
          && ( /* IS_AFFECTED( ch, AFF_SCRYING ) || -- disabled; AFF_SCRYING's bit (BV24) was later reclaimed for AFF_TRUCE (force_stun), and both are now gone entirely (v1.47/v1.48) - BV24 is unclaimed */
...
void do_practice( CHAR_DATA * ch, const char *argument )
{
   ...
   if( argument[0] == '\0' )
   {
      ...
      for( sn = 0; sn < top_sn; sn++ )
      {
         ...
         /* slice only unlocks as learnable for Swordsmen - v1.19 */
         if( sn == gsn_slice && ch->subclass != SUBCLASS_BLADEMASTER )
            continue;
         ...
         ++cnt;
         pager_printf( ch, "%18s %3d%%  ", sn == gsn_cheap ? "Cheap Attack" : skill_table[sn]->name, ch->pcdata->learned[sn] );
         if( ++col % 3 == 0 )
            send_to_pager( "\r\n", ch );
      }
   ...
   if( argument[0] == '\0' )
   {
      for( hash = 0; hash < 126; hash++ )
         for( command = command_hash[hash]; command; command = command->next )
            ...
            {
               pager_printf( ch, "%-24s", command->name );
               if( ++col % 3 == 0 )
                  send_to_pager( "\r\n", ch );
            }
      if( col % 3 != 0 )
...
void do_slist( CHAR_DATA * ch, const char *argument )
{
   ...
   if( arg1[0] != '\0' )
   {
      ...
      {
         case 'b':
            if( !str_prefix( arg1, "bounty hunting" ) )
               chosenability = HUNTING_ABILITY;
            break;
         case 'c':
            if( !str_prefix( arg1, "combat" ) )
               chosenability = COMBAT_ABILITY;
            break;
         case 'd':
            if( !str_prefix( arg1, "diplomacy" ) )
               chosenability = DIPLOMACY_ABILITY;
            break;
         case 'e':
            if( !str_prefix( arg1, "engineering" ) )
               chosenability = ENGINEERING_ABILITY;
            /* No separate ESPIONAGE_ABILITY exists in this codebase - it was
             * folded into HUNTING_ABILITY during the original port. */
            if( !str_prefix( arg1, "espionage" ) )
               chosenability = HUNTING_ABILITY;
            break;
         case 'f':
            if( !str_prefix( arg1, "force" ) )
               chosenability = FORCE_ABILITY;
            break;
         case 'g':
            if( !str_prefix( arg1, "general" ) )
               chosenability = -1;
            break;
         case 'l':
            if( !str_prefix( arg1, "leadership" ) )
               chosenability = LEADERSHIP_ABILITY;
            break;
         case 'm':
            if( !str_prefix( arg1, "medical" ) )
               chosenability = MEDICAL_ABILITY;
            break;
         case 'p':
            if( !str_prefix( arg1, "piloting" ) )
               chosenability = PILOTING_ABILITY;
            break;
         case 's':
            if( !str_prefix( arg1, "smuggling" ) )
               chosenability = SMUGGLING_ABILITY;
            break;
         default:
            chosenability = MAX_ABILITY + 1;
            break;
   ...
   lowlev = 1;
   hilev = IS_IMMORTAL( ch ) ? 200 : 150;

   if( chosenability > MAX_ABILITY )
   {
      /* First argument was not a recognized class name - treat arg1/arg2
       * as the level range, exactly as before. */
      if( arg1[0] != '\0' )
         lowlev = atoi( arg1 );
      ...
      if( hilev < lowlev )
         hilev = lowlev;
      else if( hilev > 150 && !IS_IMMORTAL( ch ) )
         hilev = 150;
      ...
      loop_start = -1;
      loop_end = MAX_ABILITY;
   }
   else
   {
      /* First argument WAS a recognized class name - arg2/arg3 become the
       * (still optional) level range instead, and the listing is
       * restricted to just this one ability. */
      if( arg2[0] != '\0' )
         lowlev = atoi( arg2 );
      ...
      loop_start = chosenability;
      loop_end = chosenability + 1;
   }
   ...
   for( ability = loop_start; ability < loop_end; ability++ )
   {
      ...
      for( i = lowlev; i <= hilev; i++ )
      {
         for( sn = 0; sn < top_sn; sn++ )
         {
            ...
            {
               pager_printf( ch, "(%3d) %-18.18s  ", i, sn == gsn_cheap ? "Cheap Attack" : skill_table[sn]->name );
               if( ++col == 3 )
               {
                  pager_printf( ch, "\r\n" );
                  col = 0;
...
void do_version( CHAR_DATA * ch, const char *argument )
{
   ...
   {
      send_to_char( "Version file not found.\r\n", ch );
      return;
   }
